/*
 * $Id: //depot/OPENDEV/sipXphone/javacore/src/org.sipfoundry/telephony/ReleaseCallStateChangePoller.java#2 $
 *
 ...
 */
public class ReleaseCallStateChangePoller extends AbstractStateChangePoller
...
////
    /**
     * Constructs the release call state change poller with the required JTAPI
     * call object and optional session.
     */
    public ReleaseCallStateChangePoller(PtCall call, PSession sessions[])
    {
        super() ;
...
////
    ...
    /**
     * Determine if there are any held connections.  If so return FALSE and
     * continuing polling, otherwise return TRUE.
     */
    public boolean poll()
    {
        boolean bHeldConnections = false ;

        synchronized (m_call)
        {
            Connection connections[] = m_call.getConnections() ;
            // Start with connection index 1, 0 is always the local connection
            for (int i=1; i<(connections.length) && (bHeldConnections == false); i++)
            {
                ...
                for (int j=0;j<tcs.length; j++)
                {
                    ...
                    {
                        bHeldConnections =  true ;
                        break ;
                    }
                }
            }
        }
        return !bHeldConnections ;
    }
