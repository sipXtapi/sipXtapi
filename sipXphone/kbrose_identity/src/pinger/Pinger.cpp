...
extern "C" STATUS dhcpcOptionGet(void *, int, int *, char *);
...
#define AUDIO_SPLASH_FILE CONFIG_PREFIX_SHARED_DATA "audioSplash.raw"
...
// kernel file uploaded via web form
#define UPGRADE_KERNEL_FILE   "/flash0/kernel.aif.install"

// jar file uploaded via web form
#define UPGRADE_JAR_FILE      "/flash0/pingtel.jar.install"

// target for jar file in upgrade process
#define PINGTEL_JAR_FILE      "/flash0/pingtel.jar"

// Should we clear the upgrade log on success?
#define CLEAR_UPGRADE_LOG     FALSE
...
#define TEST_DELAY_SHORT
   ...
   // init the phone set subsystem
   // Initialise the media buffer stuff
   // This needs to be done before the media task is started
   // mpStartUp(32000, 320);
   int maxFlowGraph;
   mConfigDb.get("PHONESET_MAX_ACTIVE_CALLS_ALLOWED", maxFlowGraph) ;
   if (maxFlowGraph <=0 ) maxFlowGraph = MAX_MANAGED_FLOW_GRAPHS;

   mpStartUp(8000, 80, 6*maxFlowGraph, &mConfigDb);


   // Should we send inband DTMF by default?
   UtlString strInBandDTMF ;
   mConfigDb.get(CONFIG_PHONESET_SEND_INBAND_DTMF, strInBandDTMF) ;
   ...
   TEST_DELAY_LONG;
   mpMediaTask = MpMediaTask::getMediaTask(maxFlowGraph); // init the media processing task

#ifdef INCLUDE_RTCP /* [ */
   TEST_DELAY_LONG;

//  Code added by DMG for the integration of  RTCP into Pingtel product

//  Create the RTC Manager object:
//  Get its RTCP Control interface, and init the Realtime Control task
      ...
      case PingerMsg::START_SIP_LINE_MGR:
         {
            ...
            mpLineMgrTask->setContactUri(Url(contactUri));
            ...
            if (mpSipUserAgentTask && OsUtil::getProductType() == OsUtil::PRODUCT_INSTANT_XPRESSA)
               {
                   ...
                   if (!mpSipUserAgentTask->isOk(OsSocket::SSL_SOCKET))
...
void Pinger::initLocalConfigFile()
{
   ...
   if(mLocalConfigDb.loadFromFile(CONFIGDB_LOCAL_IN_FLASH) == OS_SUCCESS)
   {
      ...
      UtlString gMicDevice ;

      mLocalConfigDb.get("PHONESET_AUDIO_DEVICE_CALL", gCallDevice);
      mLocalConfigDb.get("PHONESET_AUDIO_DEVICE_RINGER", gRingDevice);
      mLocalConfigDb.get("PHONESET_AUDIO_DEVICE_MIC", gMicDevice);

      DmaTask::setRingDevice(gRingDevice) ;
      DmaTask::setCallDevice(gCallDevice) ;
      DmaTask::setInputDevice(gMicDevice) ;
#endif
   }
...
void Pinger::initHttpRequestProcessor(HttpServer *pHttpServer)
{
  ...
  pHttpServer->addRequestProcessor("/cgi/filelist.cgi", processFilelist);
  pHttpServer->addRequestProcessor("/cgi/upgradelog.cgi",
    processUpgradeLog);
...
void Pinger::initRefreshMgr(OsConfigDb* config)
{
   ...
   config->get("SIP_TCP_PORT", sipTcpPort);
   config->get("SIP_UDP_PORT", sipUdpPort);
   if (sipTcpPort <= 0 && sipUdpPort <=0)
   {
      osPrintf("UDP and TCP port not set, use default %d\n", DEFAULT_SIP_PORT);
      sipUdpPort = sipTcpPort = DEFAULT_SIP_PORT;
   }
   else if (sipTcpPort <= 0)
   {
      osPrintf("TCP port not set, use the UDP port %d\n", sipUdpPort);
      sipTcpPort = sipUdpPort;
   }
   else if (sipUdpPort <= 0)
   {
      osPrintf("UDP port not set, use the TCP port %d\n", sipTcpPort);
      sipUdpPort = sipTcpPort;
   }
...
void Pinger::processFilelist(const HttpRequestContext& requestContext,
                      const HttpMessage& request,
                      HttpMessage*& response)
{
   UtlString userName("") ;
   ...
   if (userName.compareTo("admin")!=0) {
      ...
      const char * text = "<HTML><BODY>User Not Authorized<br><br>Must be user"
         " \"admin\" to access this page.</BODY></HTML>\n" ;
      HttpServer::processUserNotAuthorized(requestContext,
         request, response, text) ;
   } else {
      PingerInfo pingerInfo ;
      pingerInfo.getFlashFileSystemLocation(uriFileName) ;

      uri.append("/") ;

      // Get standard file list from HTTPServer
      HttpServer::constructFileList(fileList, uri, uriFileName) ;

      // Create Response
      HttpBody* body = new HttpBody(fileList.data(),
         fileList.length(), CONTENT_TYPE_TEXT_HTML);
      response = new HttpMessage();
      response->setResponseFirstHeaderLine(HTTP_PROTOCOL_VERSION,
         HTTP_OK_CODE, HTTP_OK_TEXT);
      response->setBody(body);
...
// Allows user to view upgrade-log
// User must be administrator in order to view this page

void Pinger::processUpgradeLog(const HttpRequestContext& requestContext,
                      const HttpMessage& request,
                      HttpMessage*& response)
{
   UtlString userName("") ;
   ...
   if (userName.compareTo("admin")!=0) {
      // Must be administrator to access page
      osPrintf("Pinger::getPingerConfigAdmin - user is not admin\n") ;
      const char * text = "<HTML><BODY>User Not Authorized<br><br>Must be user"
         " \"admin\" to access this page.</BODY></HTML>\n" ;
      HttpServer::processUserNotAuthorized(requestContext,
      ...
      // Redirect user to upgrade log page.
      response = new HttpMessage();
      response->setResponseFirstHeaderLine(HTTP_PROTOCOL_VERSION,
      HTTP_MOVED_TEMPORARILY_CODE, HTTP_MOVED_TEMPORARILY_TEXT);

      UtlString location("/upgrade-log");
      response->setLocationField(location);
      response->setContentType(CONTENT_TYPE_TEXT_HTML);
   }
}
