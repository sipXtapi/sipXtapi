...
Usage:
    ...
    Checks validity of an x509 certificate.
    ...
    Checks whether the commonName in the certificate matches the 
    server-name (by default, the full name of the current system).
    Returns a failure if these don't match.
...
then
    ...
    cert_server=`openssl x509 -in ${Certificate} -noout -subject -nameopt multiline | awk '$1 == "commonName" {print $3}'`
    if [ "${ServerName}" != "${cert_server}" ]
    then
        echo "SSL certificate commonName '${cert_server}' != '${ServerName}'" 1>&2
        ExitStatus=1
    elif [ "${Feedback}" = "VERBOSE" ]
    then
        echo "SSL certificate commonName: ${cert_server}" 
    fi
