...
OsStatus
UrlMapping::getPermissionRequired(
    ...
    ResultSet& rPermissions)
{
    ...
    currentStatus = parseHostMatchContainer(
                        requestUri,
                        registrations,
                        isPSTNNumber,
                        doTransform,
...
OsStatus
UrlMapping::parseHostMatchContainer(
    const Url& requestUri,
    ResultSet& rRegistrations,
    UtlBoolean& isPSTNNumber,
...
OsStatus
UrlMapping::parseUserMatchContainer(
    const Url& requestUri,
    ResultSet& rRegistrations,
    UtlBoolean& isPSTNNumber,
    UtlBoolean& doTransform,
    ...
    TiXmlNode* previousUserMatchNode)
{
            ...
            && (permssionMatchFound != OS_SUCCESS) )
    {
             ...
             userPatternNode = userPatternNode->NextSibling(XML_TAG_USERPATTERN ) )
        {
            ...
            TiXmlElement* userPatternElement =
                userPatternNode->ToElement();
            ...
            if(userPatternText && userPatternText->Type() == TiXmlNode::TEXT)
            {
                ...
                if (XmlUser)
                {
                    ...
                    if (userExpression.Search(testUser.data(), testUser.length()))
                    {
                       ...
                       userMatchFound = true;
                       permssionMatchFound = parsePermMatchContainer(
                          requestUri,
                          vdigits,
                          rRegistrations,
                          doTransform,
                       ...
                       isPSTNNumber = true;
                       if(permssionMatchFound == OS_SUCCESS)
                          break;
                    }
                }
            }
...
OsStatus
UrlMapping::parsePermMatchContainer(
    const Url& requestUri,
...
void
UrlMapping::convertRegularExpression(
...
void
UrlMapping::replaceAll(
    const UtlString& originalString ,
...
*/
void UrlMapping::replaceSymbols(
    const UtlString &string,
    const Url& requestUri,
    const UtlString& vdigits,
    UtlString& modifiedString)
{
