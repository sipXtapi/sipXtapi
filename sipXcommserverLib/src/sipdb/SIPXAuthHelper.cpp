    ...
    while ( keepsearching && ( result != OS_FAILED ) && (loopCount < 3) )
    {
        ...
        if ( !credentialRecordFound )
        {
            rErrorLog += (UtlString) "SIPXAuthHelper::isAuthorizedUser - " + 
                ...
                " in Credential DB\n";
            // if no domain specified, add one otherwise strip the domain off
            // provided it matches the realm name and try again
            if ( userOrExtensionAtOptDomain.index("@") == UTL_NOT_FOUND )
            {
                userOrExtensionAtOptDomain += "@" + domainName;
            } else if ( userOrExtensionAtOptDomain.index(domainName) != UTL_NOT_FOUND )
            {
                userOrExtensionAtOptDomain = 
