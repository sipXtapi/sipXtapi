...
# OpenSSL is required
#
AC_DEFUN([CHECK_SIPX_SSL],
[   AC_ARG_WITH(sipxopenssl,
                [  --with-sipxopenssl=PATH      to openssl source directory],
                [openssl_path=$withval],
                [openssl_path="/usr/local /usr/lib /usr/local/ssl /usr/ssl /usr/pkg /usr / /sw/lib"]
                )
    AC_MSG_RESULT(openssl option: ${with-sipxopenssl} withval: ${withval})
