...
if ((VXIint)(_res) != 0) { \
   fprintf(stderr, "ERROR: %s failed, error code %i, file %s, line %i\n",\
...
{
   // sInSigHandler is used to make sure we do not get into a dead loop 
   ...
   if (sig_num == SIGPIPE)
   {
      ...
      if (!sInSigHandler)
      {
         OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_CRIT, "sigHandler: caught signal: %d, ignore", sig_num);
         OsSysLog::flush();
      }
   }
   else
   {
      // Set the global shutdown flag, and exit with failure if the signal is
      // not SIGTERM.
      if (sig_num != SIGTERM)
      {
...
///////////////////////////////////////////////////////////////////////////////
static VXITRD_DEFINE_THREAD_FUNC(HeartbeatThread, userData)
{   
   const int sleepSeconds = (int)userData;

   OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG, 
         "Start HeartbeatThread, sleep seconds = %d.", sleepSeconds);

   // Block asynchronous signals, as the main thread should receive them.
   {
      sigset_t sigs_to_block;

      sigemptyset(&sigs_to_block);
      sigaddset(&sigs_to_block, SIGINT);
      sigaddset(&sigs_to_block, SIGABRT);
      sigaddset(&sigs_to_block, SIGTERM);
#if defined(__pingtel_on_posix__)
      sigaddset(&sigs_to_block, SIGHUP);
      sigaddset(&sigs_to_block, SIGQUIT);
      sigaddset(&sigs_to_block, SIGXCPU);
      sigaddset(&sigs_to_block, SIGXFSZ);
...
#endif /* defined(__pingtel_on_posix__) */

   while (TRUE)
   {
      char* pTest = (char*)malloc(10);
      if (pTest) 
      {
         free (pTest);
         pTest = NULL;

         FILE *fp = fopen("/var/tmp/mediaserverHeartbeat.dat", "r+");
         if (!fp)
         {
                 // may have failed because the monitor is deleting the file, wait and try again.
                 OsTask::delay(500);
                 fp = fopen("/var/tmp/mediaserverHeartbeat.dat", "r+");
                 if (!fp)
                 {
                         fp = fopen("/var/tmp/mediaserverHeartbeat.dat", "w+");
                 }
      ...
      {
         OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_ERR, "malloc failed, Exit HeartbeatThread");
         break;
      }
      OsTask::delay(sleepSeconds*1000);
   }
   OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_ERR, "Exit HeartbeatThread");
...
int main(int argc, char *argv[])
{
   ...
   UtlString oneCodec;
   codecFactory.buildSdpCodecFactory(codecList);
   ...
   if(value != NULL) 
   {
      maxFlowGraphs = VXIIntegerValue((const VXIInteger *) value);
   }
   ...
   // If the process is running as root and the real uid is not root
   // set the process back to running as the real UID.
   // The process was started as root to enable realtime scheduling.
   if(effectiveUid == 0 && realUid != 0)
   {
      seteuid(realUid);
   }
#endif
   ...
   if (userAgent)
   {
      userAgent->requestShutdown(); // will be deleted by CallManager
   }
   if (pCallMgr)
   {
      delete pCallMgr;
