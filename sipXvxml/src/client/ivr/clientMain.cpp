...
int main(int argc, char *argv[])
{
...
#if defined(__pingtel_on_posix__)
   // Temporarily set our UID to root in order to perform privileged
   // p_thread operations.
   ...
   if (effectiveUid == 0)
   {
      if (setuid(0) != 0)
      {
         OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_WARNING,
                       "main: Unable to setuid(0), errno = %d.  "
                       "Cannot use real-time scheduling.", errno);
      }
   }
   else
   {
      OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_WARNING,
                    "main: Effective UID upon startup is %d, not 0.  "
                    "Cannot use real-time scheduling.", effectiveUid);
   }
#endif
   ...
   // If the starting effective UID was root and the real UID was not root,
   // set the process back to running as the original real UID.
   if (effectiveUid == 0 && realUid != 0)
   {
      // Give the components time to initialize before changing UID.
      OsTask::delay(15000);  // 15 seconds
      seteuid(realUid);
   }
#endif
#endif
      ...
      const char *url)
{
   ...
   OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG, "Using VXML document '%S'\n\n", vxmlURL);
