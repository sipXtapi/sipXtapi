...
{    
        ...
        mMaxNumTransfers = mMaxNumListeners = 100;
        mMaxNumCleanups = mMaxNumCleanups = 100;
...
UtlBoolean IvrTelListener::removeFromCleanupInProgress(const char* callId)
{
   ...
   else
   {
      for (int i = 0; i < mCleanupCnt; i++)
      {
         ...
         {
            delete mpCleanupInProgress[i];
            mpCleanupInProgress[i] = NULL;
            mCleanupCnt--;
            for (int j = i; j < mCleanupCnt; j++)
            {
               mpCleanupInProgress[j] = mpCleanupInProgress[j + 1];
            }
            mpCleanupInProgress[mCleanupCnt + 1] = NULL;
            res = TRUE;
            break;
         }
      }
