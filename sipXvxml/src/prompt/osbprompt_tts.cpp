...
const char *MONTH[12] = {"january.wav", "febuary.wav", "march.wav", "april.wav",
...
#if 0
  OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
		"OSBpromptTTS::getPromptUrls: mFormat = '%s', mText = '%s'",
                mFormat.data(), mText.data());
#endif
...
#if 0
                  OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
                                "OSBpromptTTS::parse: mText = '%s', mFormat = '%s'",
                                mText.data(), mFormat.data());
#endif
              }
          }
      }
...
{
    // TODO: This code does not work for numbers greater than 9999.
    // For example, if mText=12345, then it plays:
    // 10000.wav, 2000.wav, 300.wav 45.wav. Instead this should be:
    // 12000.wav, 300.wav, 45.wav
    OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
  ...
  if (length <= 0)
    ...
    if( num <= 100 )
    {
        mPrompts[0] = Url(mBaseUrl + mText + ".wav");
        OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
                      "OSBpromptTTS::getNumberUrls: mPrompts[0] = '%s'",
                      mPrompts[0].toString().data());
    }
    else
    {
      mCount = length - 1;
        // Get the last two digits.
        UtlString strLastTwoDigits = mText( index, 2 );
        
        mPrompts[index] = Url(mBaseUrl + strLastTwoDigits + ".wav");
        OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
                      "OSBpromptTTS::getNumberUrls: mPrompts[%d] = '%s'",
                      index, mPrompts[index].toString().data());
        ...
        while( index > 0 )
        {
            ...
            mText = mText(0, index ) ;
            mPrompts[index] = Url(mBaseUrl + digit + ".wav");
            OsSysLog::add(FAC_MEDIASERVER_VXI, PRI_DEBUG,
                          "OSBpromptTTS::getNumberUrls: mPrompts[%d] = '%s'",
                          index, mPrompts[index].toString().data());
        }
...
void OSBpromptTTS::getOrdinalUrlHelper(UtlString& text, UtlString& url)
{
    ...
    // Stores the last digits for numbers greater than 10.
    UtlString lastDigit;

    if (length > 1)
    {
        // If mText=23, then last digit = 3.
        // This will determine the ordinal - (1)st, (2)nd, (3)rd or (4-9)th.
        lastDigit = ((const char*)text)[length-1] ;
    }
    else
    {
        lastDigit = text;
