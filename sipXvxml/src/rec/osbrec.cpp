...
static VXIrecResult waitForResponse(VXIrecInterface *pThis,
                                        OSBQueuePlayerListener* pListener,
    ...
    do
    {
      ...
      if (waitRet == OS_SUCCESS) 
      {
        ...
        else
        {
          ...
          tp->Diag(DIAG_TAG_REC, NULL, L"Rec waitForResponse dtmf event, key pressed (%d)", isFirstKey);
          break;
        }
      }
      else 
              ...
              int isFirstKey)
{
  ...
  /* UNUSED VARIABLE */ OSBrecImpl* impl = reinterpret_cast<OSBrecImpl*>(pThis);
                                    ...
                                    VXIrecRecognitionResult **recogResult)
{
  ...
  *recogResult = result;
  ...
  res = VXIMapCreate();
   ...
   OSBQueuePlayerListener* pListener = VXIGetPlayerListener((const char*)impl->callId, (unsigned)impl->channel);        
