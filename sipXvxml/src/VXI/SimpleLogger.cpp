...
public:
  ...
  { if (buffer.str().empty()) return;
    log->Diagnostic(log, MESSAGE_BASE + id, NULL, L"%s", buffer.str().c_str()); }

  virtual void LogDiagnostic(TAGID tagID, const wchar_t * text) const
  { log->Diagnostic(log, MESSAGE_BASE + tagID, NULL, L"%s", text); }
