...
/* ============================ MANIPULATORS ============================== */
     ...
     * \param bufferSize - this is the size of the chunks of data to be read off the socket.  It is not the total size of the message read.  Chunks of this size are read off and accumulated as the whole message.
     * \param guessMessageSize - this is a guess at the total size of the message to be read in.  This is a soft limit.  It does not restrict the maximum size of the message.  This is an optimization to minimize the memory shuffling in appending and copying the read buffer to the container for the whole message.
     * \param maxContentLength - this is the maximum message size that will be read.  If the message content-length is larger the socket is closed and the buffer is cleared.
	 * \return the number of bytes read from the socket
     */
	int read(OsSocket* inSocket, int bufferSize = HTTP_DEFAULT_SOCKET_BUFFER_SIZE,
             UtlString* externalBufer = NULL,
...
/* ============================ ACCESSORS ================================= */
    ...
    */
     ...
     * sent so that it may be resent or timedout
     */
    //@{
	...
	//!  Used for transport reliablity to track the last duration
	//! waited before resending
	void setResendDuration(int resendMSec);
	int getResendDuration() const;

	// Used by transport reliablity to track the number of times the
	// message was send (and resent).
	int getTimesSent() const;
	void incrementTimesSent();
	void setTimesSent(int times = 0);

	// Used by the tranport reliablity to track which protocol the
    ...
    //! Find the number of occurance of the given name in the header
