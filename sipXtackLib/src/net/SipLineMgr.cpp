...
UtlBoolean
SipLineMgr::addLine(SipLine& line, UtlBoolean doEnable)
{
...
SipLineMgr::enableLine(const Url& identity)
{
         ...
         line->getState() != SipLine::LINE_STATE_TRYING )
    {
        ...
        if (!mpRefreshMgr->newRegisterMsg(canonical,line->getLineId()))
        {
...
void SipLineMgr::setContactUri(const Url& contactUri)
{
        ...
        if (pConfigDb->get(strKey, strUrl))
        {
            if (!strUrl.isNull())
            {
                ...
                if( address.isNull())    //dynamic IP address
                {
                    ...
                    mContactUri.getHostAddress(contactHost);
                    int contactPort = mContactUri.getHostPort();
                ...
                //get line ID
                /*strKey = strSubKey;
                strKey.append(LINE_PARAM_LINEID);
                pConfigDb->get(strKey, strValue);
                line.setLineId(strValue);*/
