...
#if defined(_WIN32)
#	include "resparse/wnt/nterrno.h" /* Must add to Visual Studio's errno.h --GAT */
...

{
...
#ifndef NO_SSL_LIB
   	mSipTlsServer = new SipTlsServer(mTlsPort, this);
	mSipTlsServer->startListener();
#endif
   	mSipTcpServer = new SipTcpServer(mTcpPort, this);
   ...
   mTransactionStateTimeoutMs = 10 * mLastResendTimeoutMs;
	...
	if(mTcpPort > 0 && mTcpPort != mUdpPort)
	{
		SipMessage::buildSipUrl(&mContactAddress, hostIpAddress.data(),
			(mTcpPort == SIP_PORT) ? 0 : mUdpPort,
			SIP_TRANSPORT_TCP,
			defaultSipUser.data());
#ifdef TEST_PRINT
    ...
    //osPrintf("SipUserAgent::SipUserAgent initializing SipTransaction::smBranchIdBase %s\n", SipTransaction::smBranchIdBase.data());
    ...
    if(natPingUrl && *natPingUrl && natPingFrequency > 0)
    {
        ...
        //osPrintf("UDP Server started URL: %s frequency: %d method: %s\n",
        //    natPingUrl, natPingFrequency, natPingMethod ? natPingMethod : "");
    }
...
# define VENDOR "sipX"
    ...
    OsQueuedEvent* queuedEvent = new OsQueuedEvent(*incomingQ, 0);

    OsTimer* timer = new OsTimer(*queuedEvent);
    // Convert from mSeconds to uSeconds
    OsTime lapseTime(0,
        mTransactionStateTimeoutMs * 1000);
   ...
   mLastCleanUpTime = time.seconds();
	//mLastCleanUpTime = OsDateTime::getSecsSinceEpoch();
...
SipUserAgent::~SipUserAgent()
{
    ...
    mUserAgentHeaderProperties.destroyAll();
}
    ...
    while ((pObserver = (SipObserverCriteria*) iterator()))
    {
        if (pObserver->getObserverQueue() == &messageQueue)
        {
                    ...
                    (pObserverData == pObserver->getObserverData()))
            {
                ...
                mMessageObservers.destroy(pObserver);
            }
        }
    }
...
void SipUserAgent::allowMethod(const char* methodName)
{
    if(methodName)
    {
                            ...
                            void* responseListenerData)
{
	...
	else
	{
        ...
        else
        {
            ...
            //osPrintf("WARNING: SipUserAgent::send message being resent\n");
        }
	}
    ...
    //{
    //    UtlString hash;
    //    SipTransaction::buildHash(message, TRUE, hash);

        // If we could not lock the transaction it was removed from
        // the list
    //    if(! mSipTransactions.waitUntilAvailable(transaction, hash))
    //        transaction = NULL;
    //}
    ...
    if(transaction)
    {
        ...
        else
        {
            ...
            {
                // Should not be getting here
                OsSysLog::add(FAC_SIP, PRI_WARNING, "SipUserAgent::send %s request matches existing transaction\n", method.data());
                //osPrintf("WARNING SipUserAgent::send %s request matches existing transaction\n", method.data());
    ...
    if(transaction == NULL)
    {
        if(isResponse)
        {
            ...
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipUserAgent::send response without an existing transaction\n");
            //osPrintf("WARNING: SipUserAgent::send response without an existing transaction\n");
        }
        ...
        else
        {
            ...
            if(message.getViaField(&viaField, 0))
            {
                ...
                //{
                //    UtlString parentHash;
                //    SipTransaction::buildHash(message,
                //                              FALSE, // incoming
                //                              parentHash);

                    // If we cannot lock it, it does not exist
                //    if(!mSipTransactions.waitUntilAvailable(parentTransaction,
                //                                        parentHash))
            ...
            //SipMessage* messageCopy = new SipMessage(message);
		    ...
		    // Temporary fix for release 2.6 -- do not copy to release 2.7!
                    OsSysLog::add(FAC_SIP, PRI_INFO,
                       "SipUserAgent::send proxied client transaction not part of server transaction, parent relationship: %s\n",
                       parentRelationshipString.data());
                    //osPrintf("WARNING: SipUserAgent::send proxied client transaction not part of server transaction\n");
                    if(parentTransaction)
            ...
            else if(!isUaTransaction)
            {
                ...
                //osPrintf("DEBUG: SipUserAgent::send proxied client transaction does not have parent\n");
            }
    ...
    if(transaction)
    {
        ...
        if(isUaTransaction)
        {
	        ...
	        if(uaName.isNull())
	        {
                ...
                if ( mUserAgentHeaderProperties.entries() )
                {
                    UtlSListIterator iterator(mUserAgentHeaderProperties);
                    UtlString* property;

                    while ((property = (UtlString*)iterator()))
                    {
                        uaName += " ";
                        uaName += property->data();
                    }
                }
                ...
                uaName.append(" (WinNT)");
#elif defined(_VXWORKS)
                uaName.append(" (VxWorks)");
#elif defined(__linux__)
                uaName.append(" (Linux)");
#elif defined(sun)
                uaName.append(" (Solaris)");
#endif
			...
			UtlString allowedMethodsSet;
 			if (!message.getAllowField(allowedMethodsSet) &&
 				(method.compareTo(SIP_REFER_METHOD) == 0 || method.compareTo(SIP_INVITE_METHOD) == 0) )
 			{
 				UtlString allowedMethods;
 				getAllowedMethods(&allowedMethods);
 				message.setAllowField(allowedMethods);
            ...
            // There seems to be a move to making contact manditory in INVITE
            ...
            if((method.compareTo(SIP_INVITE_METHOD) == 0 ||
                ...
                !message.getContactUri(0, &contactField))
            {
                OsSysLog::add(FAC_SIP, PRI_WARNING,"Contact field added in SipUserAgent::send for %s request\n",
                    method.data());

                //osPrintf("WARNING: Contact field added in SipUserAgent::send for %s request\n",
                //    method.data());
            ...
            // If we are requiring symetrical authentication - Stuff in the authenticate field
    ...
    else
    {
        ...
        //osPrintf("SipUserAgent::send failed to construct new transaction\n");
    }

    if(transaction) mSipTransactions.markAvailable(*transaction);
    //mSipTransactions.unlock();
								...
								int port)
{
	...
	if(sentOk)
	{
		...
		char buff[10];
		sprintf(buff, "%d",port == 0? 5060:port);
	...
	else
	{
        ...
        OsSysLog::add(FAC_SIP_OUTGOING, PRI_DEBUG, "%s", messageStatusString.data());
								...
								int port)
{
	...
	else
	{
                ...
                OsSysLog::willLog(FAC_SIP_OUTGOING, PRI_INFO))
     {
	    ...
	    messageStatusString.append("---- Port: ");
	    char buff[10];
	    sprintf(buff, "%d",port == 0? 5061:port);
	    messageStatusString.append(buff);
	    messageStatusString.append("----\n");
...
void SipUserAgent::dispatch(SipMessage* message, int messageType)
{
	...
	if(messageType == SipMessageEvent::APPLICATION)
	{
        ...
        if(transaction == NULL)
        {
            if(isResponse)
            {
               OsSysLog::add(FAC_SIP, PRI_WARNING,"SipUserAgent::dispatch received response without transaction\n");
               //osPrintf("SipUserAgent::dispatch received response without transaction\n");

#ifdef TEST_PRINT
               if (OsSysLog::willLog(FAC_SIP, PRI_DEBUG))
               {
                  OsSysLog::add(FAC_SIP, PRI_DEBUG,"=Response w/o request=>\n%s\n======================>\n", msgBytes.data());
                  //osPrintf("=Response w/o request=>\n%s\n======================>\n", msgString.data());

                  UtlString transString;
                  mSipTransactions.toStringWithRelations(transString, *message, FALSE);
                  OsSysLog::add(FAC_SIP, PRI_DEBUG,"Transaction list:\n%s\n===End transaction list===\n", transString.data());
                  //osPrintf("Transaction list:\n%s\n===End transaction list===\n", transString.data());
               }
#endif
               ...
               if(callId.index("-ping@") > 0)
               {
                   ...
                   if(receivedSet && receivedPortSet)
                   {
                       ...
                       OsSysLog::add(FAC_SIP, PRI_DEBUG,"SipUserAgent:dispatch set new contact: %s\n",
                           mContactAddress.data());
                       //osPrintf("SipUserAgent:dispatch set new contact: %s\n",
                       //    mContactAddress.data());
                   }
				   mPingLock = FALSE;
               }
            ...
            else
            {
                ...
                if(method.compareTo(SIP_ACK_METHOD) == 0)
                {
                    ...
                    OsSysLog::add(FAC_SIP, PRI_DEBUG,"NOTE: SipUserAgent::dispatch received ACK without transaction\n");
                    //osPrintf("NOTE: SipUserAgent::dispatch received ACK without transaction\n");
                }
                else if(method.compareTo(SIP_CANCEL_METHOD) == 0)
                {
                    relationship = SipTransaction::MESSAGE_CANCEL;
                    OsSysLog::add(FAC_SIP, PRI_WARNING, "SipUserAgent::dispatch received CANCEL without transaction\n");
                   // osPrintf("WARNING: SipUserAgent::dispatch received CANCEL without transaction\n");
                }
                else
                {
                    relationship = SipTransaction::MESSAGE_REQUEST;
        ...
        {
            if(relationship == SipTransaction::MESSAGE_FINAL ||
                ...
                relationship == SipTransaction::MESSAGE_CANCEL_RESPONSE)
            {
                   ...
                   (shouldDispatch || delayedDispatchMessage))
		       {
                   ...
                   if(responseQ  && delayedDispatchMessage)
                   {
				       ...
				       tempDelayedDispatchMessage->setResponseListenerData(request->getResponseListenerData() );
		   ...
		   // if this is a request check if it is supported
            else if(relationship == SipTransaction::MESSAGE_REQUEST)
		   {
               ...
               // NOTE: The stripping should all be done in
               // the accessor utilities
			   //delete leading and trailing white spaces
			   //method = method.strip(UtlString::both);
			   //toAddress = toAddress.strip(UtlString::both);
			   //fromAddress = fromAddress.strip(UtlString::both);
			   //uriAddress = uriAddress.strip(UtlString::both);
			   //protocol = protocol.strip(UtlString::both);
			   //sipVersion = sipVersion.strip(UtlString::both);
			   //seqMethod = seqMethod.strip(UtlString::both);
			   //callIdField = callIdField.strip(UtlString::both);
                   ...
                   shouldAuthenticate(message))
               {
                   if(!authorized(message))
                   {
                       ...
                       response->setRequestUnauthorized(message,
                               ...
                               "1234567890", // :TBD: nonce should be generated by SipNonceDb
                               "abcdefghij"  // opaque
                               );
                       // I think this is not suppose to be here
                       //send(response);
                       //delete message;
                       //message = NULL;
                   }
           ...
           else if(relationship == SipTransaction::MESSAGE_ACK ||
           ...
           else if(relationship == SipTransaction::MESSAGE_NEW_FINAL)
           {
               // Forward it on to interested applications
             SipMessage* request = transaction->getRequest();
             shouldDispatch = TRUE;
           ...
           else
           {
              if (OsSysLog::willLog(FAC_SIP, PRI_WARNING))
              {
                 ...
                 OsSysLog::add(FAC_SIP, PRI_WARNING, "SipUserAgent::dispatch unhandled incoming message: %s\n",
                       relationString.data());
                 //osPrintf("WARNING: SipUserAgent::dispatch unhandled incoming message: %s\n",
                 //    relationString.data());
              }
           }
        }
                                           ...
                                           int messageType)
{
	...
	// Other response get sent to the listener
	//if(messageListener)
	{
#ifdef TEST
        osPrintf("SipUserAgent::dispatch posting message status type: %d\n",
            messageType);
#endif
        ...
        // Add the event to the listener's queue
        // messageListener->postMessage(event);
                                                     ...
                                                     const char* method)
{
    ...
    // lock the message observer list
    {
        ...
        do
        {
            ...
            if(observerCriteria &&
                !message->isResponse() &&
                (strcmp(method, SIP_SUBSCRIBE_METHOD) == 0 ||
                 strcmp(method, SIP_NOTIFY_METHOD) == 0))
            {
                UtlString criteriaEventName;
                observerCriteria->getEventName(criteriaEventName);
                // If the eventName is empty take all requests or
                // if the event type matches
                if(criteriaEventName.isNull() ||
                   messageEventName.index(criteriaEventName, 0, UtlString::ignoreCase) == 0)
                {
                    matchEvent = TRUE;
                }
                else
            ...
            {
                // Do not care if it matches it is not a SUBSCRIBE or NOTIFY request
                matchEvent = TRUE;
            }

            // Check to see if the session criteria matches
            if ((observerCriteria != NULL) && (message != NULL) && matchEvent)
            {
                SipSession* pCriteriaSession = observerCriteria->getSession() ;
                if ((pCriteriaSession != NULL) && !pCriteriaSession->isSameSession((SipMessage&) *message))
                {
                    matchEvent = FALSE;
                }
            }
		...
		{
                   OsSysLog::add(FAC_SIP, PRI_ERR, "SipUserAgent::queueMessageToInterestedObservers - observer Q is full (numMsgs=%d), message is not posted.", numMsgs);
                   osPrintf("SipUserAgent::queueMessageToInterestedObservers - observer Q is full (numMsgs=%d), message is not posted.", numMsgs);
                }
            }
        }
...
UtlBoolean SipUserAgent::handleMessage(OsMsg& eventMessage)
{
	...
	// && msgSubType == CP_SIP_MESSAGE)
	{
		...
		if(sipMsg)
        {
            ...
            //osPrintf("SipUserAgent::handleMessage posting message\n");
            // messageListener->postMessage(eventMessage);
        ...
        if(sipEvent)
        {
            ...
            if(msgEventType == SipMessageEvent::TRANSACTION_RESEND)
            {
		        if(sipMessage)
                {
                    ...
                    // is a message.  If there is no message, this is the
                    // periodic garbage collection timer
                    if(timer)
		            {
			            delete timer;
			            timer = NULL;
                    ...
                    {
                        osPrintf("SipUserAgent::handleMessage resend Timeout message with NULL transaction\n");
                    }
                    ...
                    //{
                    //    UtlString hash;
                    //    SipTransaction::buildHash(*sipMessage, TRUE, hash);
                        // If we cannot lock it, it does not exist
                    //    if(!mSipTransactions.waitUntilAvailable(transaction, hash))
                    //        transaction = NULL;
                    //}
                    ...
                    if(transaction)
                    {
                        ...
                        if(nextTimeout == 0)
                        {
                            if (OsSysLog::willLog(FAC_SIP, PRI_DEBUG))
                            {
                                ...
                                transactionString.insert(0, "SipUserAgent::handleMessage timeout send failed\n");
                    ...
                    else
                    {
                        ...
                        //osPrintf("ERROR: SipUserAgent::handleMessage SIP message timeout expired with no matching transaction\n");
                        ...
                        // Need to free up the message. (It is attached to
                        // the SIpMessageEvent which will free it
                        //delete sipMessage;
                        //sipMessage = NULL;
                    }
                    //mSipTransactions.unlock();
            ...
            else if(msgEventType == SipMessageEvent::TRANSACTION_EXPIRATION)
            {
                ...
                if(sipMessage)
                {
                    ...
                    // Could not find a transaction for this exired message
                    else
                    {
                        if (OsSysLog::willLog(FAC_SIP, PRI_DEBUG))
                        {
                            ...
                            OsSysLog::add(FAC_SIP, PRI_DEBUG,
                                "SipUserAgent::handleMessage event timeout with no matching transaction: %s",
                                noTxMsgString.data());
                        }
                    }
                }
            ...
            else
            {
               OsSysLog::add(FAC_SIP, PRI_WARNING,
                   "SipUserAgent::handleMessage unknown timeout event: %d.\n", msgEventType);
...
void SipUserAgent::garbageCollection()
{
   ...
   long bootime = time.seconds();
	...
	{
#ifdef TEST_PRINT
        OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipUserAgent::garbageCollection bootime: %d then: %d tcpThen: %d oldTransaction: %d oldInviteTransaction: %d\n",
                bootime, then, tcpThen, oldTransaction, oldInviteTransaction);
#endif

		//sentMessages.removeOldMessages(then);
		//receivedMessages.removeOldMessages(then);
        //mSipTransactions.lock();
        mSipTransactions.removeOldTransactions(oldTransaction,
                                               oldInviteTransaction); //temp double of transaction life
#ifdef TEST_PRINT
        osPrintf("SipUserAgent::garbageCollect GC of transaction list\n");
#endif
        //mSipTransactions.unlock();

#ifdef TEST_PRINT
        osPrintf("SipUserAgent::garbageCollect begin GC of sipclients\n");
#endif
...
#endif
#ifdef TEST_PRINT
        osPrintf("SipUserAgent::garbageCollect end GC of sipclients\n");
#endif
...
void SipUserAgent::setUserAgentHeaderProperty( const char* property )
{
    if ( property )
    {
        mUserAgentHeaderProperties.append( new UtlString( property ));
    }
}
...
void SipUserAgent::setMaxForwards(int maxForwards)
{
    ...
    else
    {
       ...
       // osPrintf("SipUserAgent::setMaxForwards maxForwards <= 0: %d\n",
       //     maxForwards);
    }
}
...
int SipUserAgent::getMaxForwards()
{
    ...
    if(mMaxForwards <= 0)
    {
        OsSysLog::add(FAC_SIP, PRI_DEBUG,"SipUserAgent::getMaxForwards maxForwards <= 0: %d\n",
            mMaxForwards);
        //osPrintf("SipUserAgent::getMaxForwards maxForwards <= 0: %d\n",
        //    mMaxForwards);
                              ...
                              int& port)
{
    ...
    {
        port = mTcpPort == SIP_PORT ? 0 : mTcpPort;
    }
#ifdef TLS
    else if(protocol == OsSocket::SSL_SOCKET)
    {
        port = mTlsPort == SIP_TLS_PORT ? 0 : mTlsPort;
    }
#endif
    else
    {
...
void SipUserAgent::setDefaultExpiresSeconds(int expiresSeconds)
{
    ...
    {
        OsSysLog::add(FAC_SIP, PRI_ERR, "SipUserAgent::setDefaultExpiresSeconds illegal expiresSeconds value: %d IGNORED\n",
            expiresSeconds);
    }
}
...
void SipUserAgent::setDefaultSerialExpiresSeconds(int expiresSeconds)
{
    ...
    {
        OsSysLog::add(FAC_SIP, PRI_ERR, "SipUserAgent::setDefaultSerialExpiresSeconds illegal expiresSeconds value: %d IGNORED\n",
            expiresSeconds);
    }
}
...
void SipUserAgent::setMaxTcpSocketIdleTime(int idleTimeSeconds)
{
    ...
    {
        OsSysLog::add(FAC_SIP, PRI_ERR, "SipUserAgent::setMaxTcpSocketIdleTime idleTimeSeconds: %d less than mMinInviteTransactionTimeout: %d IGNORED\n",
...
#endif
        default :
            osPrintf("Invalid socket type passed to SipUserAgent::isOK");
            break;
    }
...
UtlBoolean SipUserAgent::shouldAuthenticate(SipMessage* message) const
{
	...
	if ( strcmp(method.data(), SIP_ACK_METHOD) == 0 || strcmp(method.data(), SIP_CANCEL_METHOD) == 0)
	{
		methodCompare = FALSE;
	}
...
void SipUserAgent::lookupSRVSipAddress(UtlString protocol, UtlString& sipAddress, int& port)
{
    ...
    if (sipIpAddress != "127.0.0.1")
    {
        ...
        else
        {
            ...
            SipSrvLookup::initServerT(toServerUdp);
            SipSrvLookup::initServerT(toServerTcp);
            int i;
            ...
            for (i=0; SipSrvLookup::isValidServerT(server_list[i]); i++)
            {
                if (SipSrvLookup::getProtocolFromServerT(server_list[i]) ==
                    OsSocket::UDP)
                {
                    if (! SipSrvLookup::isValidServerT(toServerUdp))
                    {
                ...
                else if (SipSrvLookup::getProtocolFromServerT(server_list[i]) ==
                    OsSocket::TCP)
                {
                    if (SipSrvLookup::isValidServerT(toServerTcp))
                    {
            ...
            if (!protocol.compareTo("TCP") &&
                SipSrvLookup::isValidServerT(toServerTcp))
            {
                int newPort = SipSrvLookup::getPortFromServerT(toServerTcp);
                if (newPort > 0)
                {
                    SipSrvLookup::getIpAddressFromServerT(toServerTcp, sipAddress);
                    port = newPort;
                }
                OsSysLog::add(FAC_SIP, PRI_DEBUG,"SipUserAgent:: found TCP server %s port %d\n", sipAddress.data(), newPort);
                //osPrintf("found TCP server %s port %d\n", sipAddress.data(), newPort);
            }
            else if (SipSrvLookup::isValidServerT(toServerUdp))
            {
                int newPort = SipSrvLookup::getPortFromServerT(toServerUdp);
                if (newPort > 0)
                {
                    SipSrvLookup::getIpAddressFromServerT(toServerUdp, sipAddress);
                    port = newPort;
                }
#ifdef TEST_PRINT
            ...
            SipSrvLookup::freeServerT(server_list);
        }
    }
}
