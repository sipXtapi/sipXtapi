                   ...
                   enum SipTransaction::messageRelationship& relationship)
{
    ...
    while ((transactionFound = (SipTransaction*) iterator()))
    {
        ...
        if(messageTransaction &&
            transactionFound != messageTransaction)
                continue;
        ...
        if(messageTransaction == NULL && // this message does not point to this TX
            ((transactionFound->getState()) ==
                SipTransaction::TRANSACTION_LOCALLY_INIITATED))
        {
            continue;
        }

        // The DNS SRV parent client transaction should never have
        // any incoming messages that match it as its requests
        // should never actually be sent.
        //if(messageTransaction == NULL && // this message does not point to this TX
        //    !isOutgoing &&
        //    transactionFound->isServerTransaction() == FALSE &&
        //    transactionFound->isDnsSrvChild() == FALSE)
        //{
        //    continue;
        //}
    ...
    OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransactionList::findTransactionFor relation: %s\n%s",
        relationString.data(), bytes.data());
#endif
    ...
    if(transactionFound && isBusy)
    {
        ...
        if(!waitUntilAvailable(transactionFound, callId))
        {
            if (OsSysLog::willLog(FAC_SIP, PRI_WARNING))
            {
                UtlString relationString;
                SipTransaction::getRelationshipString(relationship, relationString);
                OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransactionList::findTransactionFor transaction %p not available relation: %s",
