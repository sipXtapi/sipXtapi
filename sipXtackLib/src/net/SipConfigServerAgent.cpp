...
SipConfigServerAgent* SipConfigServerAgent::startAgents(const char* configFileName)
{
    ...
    {
        configDb.set("SIP_SDS_UDP_PORT", "");
        configDb.set("SIP_SDS_TCP_PORT", "");
    ...
    configDb.get("SIP_SDS_UDP_PORT", sipTcpPort);
    if(sipTcpPort <= 0) sipTcpPort = 5090;

    configDb.get("SIP_SDS_TCP_PORT", sipUdpPort);
    if(sipUdpPort <= 0) sipUdpPort = 5090;
