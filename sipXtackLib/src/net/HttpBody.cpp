...
HttpBody::HttpBody(const char* bytes, int length, const char* contentType)
{
   for(int partIndex = 0; partIndex < MAX_HTTP_BODY_PARTS; partIndex++)
   {
       mpBodyParts[partIndex] = NULL;
        ...
        if (contentType)
        {
                mBodyContentType.append(contentType);
        NameValueTokenizer::frontBackTrim(&mBodyContentType, " \t");
        //osPrintf("Content type: \"%s\"\n", mBodyContentType.data());
        int boundaryIndex = mBodyContentType.index(MULTIPART_BOUNDARY_PARAMETER,
            0, UtlString::ignoreCase);

        if(boundaryIndex >=0 &&
            mBodyContentType.index(CONTENT_TYPE_MULTIPART,
            0, UtlString::ignoreCase) == 0)
        {
            ...
            int fieldLength = mBodyContentType.length();
            while(boundaryIndex < fieldLength &&
                (mBodyContentType.data()[boundaryIndex] == ' ' ||
                mBodyContentType.data()[boundaryIndex] == '\t'))
                boundaryIndex++;

            if(mBodyContentType.data()[boundaryIndex] == '=')
            {
                mMultipartBoundary.append(&mBodyContentType.data()[boundaryIndex + 1]);
...
HttpBody::HttpBody(const HttpBody& rHttpBody)
{
        ...
        mBodyContentType = rHttpBody.mBodyContentType;
...
HttpBody::operator=(const HttpBody& rhs)
{
   if (this == &rhs)            // handle the assignment to self case
      return *this;
        ...
        mBodyContentType = rhs.mBodyContentType;
