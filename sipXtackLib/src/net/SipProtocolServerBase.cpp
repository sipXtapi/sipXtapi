                            ...
                            int hostPort)
{
    ...
    // We can have multiple creates and sends at a time
    // The list protects itself from add collisions
    // However as we are taking a pointer out of the list
    // We cannot allow deletes until we are done with the list
    //clientLock.acquireRead();
    ...
    if(client)
    {
        ...
        // A window exists here for another thread to delete
        // this client out from under us.
        // We use clientLock to prevent this

        sendOk = client->send(message);

        //clientLock.releaseRead();
        ...
        if(!sendOk)
        {
            ...
            if (clientTaskId != callingTaskId)
            {
				...
				deleteClient(client);
				client = NULL;
				//Do not need to clientLock.releaseWrite();
            }
        }
    }
                                      ...
                                      int hostPort)
{
    ...
    if(! client)
    {
           ...
           isReadyToWrite)
        {
            ...
            // UDP clients do not run (i.e. read messages)
