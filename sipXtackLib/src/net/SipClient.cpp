...
SipClient::~SipClient()
{
	...
	if(clientSocket)
	{
       ...
       {
           UtlString remoteSocketHost;
           clientSocket->getRemoteHostName(&remoteSocketHost);
/*	DWW 	   osPrintf("SipClient::~SipClient socket descriptor: %d host: %s port: %d type: %d CallId: %s \n",
			    clientSocket->getSocketDescriptor(),
                remoteSocketHost.data(),
                clientSocket->getRemoteHostPort(), clientSocket->getIpProtocol(),
                callIdLabel.data());
*/
       }
#endif
...
UtlBoolean SipClient::send(SipMessage* message)
{
   ...
   osPrintf("BEGIN SipClient::send\n");
	...
	if(clientSocket)
	{
		...
		// Try to set up a new socket and resend
		/*else if(!clientSocket->isOk())
		{
			clientSocket->reconnect();
			bytesSent = message->write(clientSocket);
			if(bytesSent > 0)
			{
				sendOk = TRUE;
			}
                            ...
                            int port)
{
        ...
        clientSocket->getIpProtocol() == OsSocket::UDP)
    {
        ...
        // Wait until the socket is ready to write
        if(clientSocket->isReadyToWrite(-1))
        {
            //Lock to prevent multitreaded read or write
            mSocketLock.acquire();
