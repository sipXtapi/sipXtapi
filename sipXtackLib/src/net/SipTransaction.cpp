                               ...
                               UtlBoolean userAgentTransaction)
{
   ...
   if(request)
   {
       ...
       // Set the hash key
       //this->append(mCallId);
       //this->append(mIsServerTransaction ? 's' : 'c');
       ...
       else
       {
           // Do not attach it here as it will get passed in
           // later for handleOutgoing or handleIncoming
           /*int responseCode = request->getResponseStatusCode();
           // Provisional
           if(responseCode < SIP_2XX_CLASS_CODE)
           {
               mpLastProvisionalResponse = request;
           }
           else if(responseCode >= SIP_2XX_CLASS_CODE)
           {
               mpLastFinalResponse = request;
           }*/
       }
       ...
       // If this is a new client transaction
       if(!mIsServerTransaction)
       {
           // We need to create a new branch id
           getNewBranchId(*request, mBranchId);
       }
       // This is a server transaction, the branch id is
...
SipTransaction::~SipTransaction()
{
    ...
    if(mWaitingList)
    {
        ...
        if(numEvents > 0)
        {
            ...
            OsSysLog::add(FAC_SIP, PRI_ERR, "SipTransaction::~SipTransaction %d waiting events in list\n",
                numEvents);
        }
                    ...
                    UtlString& branchId)
{
    ...
    if(smBranchIdBase.isNull())
    {
        ...
        smBranchIdBase.append(':');
        int ranNum = rand();
        sprintf(buf, "%d", ranNum);
        smBranchIdBase.append(buf);

        //osPrintf("SipTransaction::getNewBranchId initializing smBranchIdBase %s\n", smBranchIdBase.data());

    }

    UtlString branchSeed;
    request.getCallIdField(&branchSeed);
    branchSeed.append(smBranchIdBase);
    sprintf(buf, "%d", transNum);
    branchSeed.append(buf);
    NetMd5Codec::encode(branchSeed.data(), branchId);

    branchId.insert(0, BRANCH_ID_PREFIX); // Magic cookie must be in front
}
                            ...
                            enum messageRelationship relationship) // casual/serious
{
    ...
    case MESSAGE_REQUEST:
        ...
        {
            //delete mpRequest;
           OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::addResponse add of request to existing transaction, IGNORED\n");
        }
        else
        {
            mpRequest = response;
    ...
    case MESSAGE_2XX_ACK:
        if(mpAck)
        {
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::addResponse ACK already exists, IGNORED\n");
        }
        else
        {
            mpAck = response;
    ...
    case MESSAGE_CANCEL:
        if(mpCancel)
        {
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::addResponse CANCEL already exists, IGNORED\n");
        }
        else
        {
            mpCancel = response;
    ...
    case MESSAGE_CANCEL_RESPONSE:
        if(mpCancelResponse)
        {
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::addResponse CANCEL response already exists, IGNORED\n");
        }
        else
        {
    ...
    default:
        OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::addResponse message with bad relationship: %d\n",
            relationship);
        break;
    }
                                         ...
                                         enum messageRelationship relationship)
{
    ...
    if(relationship == MESSAGE_DUPLICATE)
    {
        ...
        {
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::handleOutgoing send of duplicate message\n");
        }
        ...
        (method.compareTo(SIP_CANCEL_METHOD) == 0))
    {
        if (OsSysLog::willLog(FAC_SIP, PRI_WARNING))
        {
            ...
            OsSysLog::add(FAC_SIP, PRI_WARNING,
                "SipTransaction::handleOutgoing should not send CANCEL on DNS parent\n%s\n%s",
                requestString.data(),
                transString.data());
        }
    }
    ...
    else
    {
        ...
        // This is done in doFirstSend now as it must occur before
        // the send to avoid the race of a response coming in before
        // the request has been added to the transaction and the copy
        // of the message cannot be done before setting the transport information.
        //if(relationship == MESSAGE_REQUEST ||
        ...
        //{
            // Make a copy to attach to the transaction
            //SipMessage* messageCopy = new SipMessage(outgoingMessage);

            //addResponse(messageCopy,
            //            TRUE, // outgoing
            //            relationship);
        //}
                                           ...
                                           enum OsSocket::SocketProtocolTypes& toProtocol)
{
    ...
    else
    {
...
#ifdef TEST_PRINT
        osPrintf("SipTransaction::prepareRequestForSend got proxy address: \"%s\"\n", toAddress.data());
#endif
        ...
        //request.getRequestMethod(&method);
#ifdef TEST_PRINT
        osPrintf("SipTransaction::prepareRequestForSend getting first route uri: %s\n", routeUri.data());
#endif
        ...
        else //if(!routeAddress.isNull())
        {
            ...
            if(!nextHopLooseRoutes)
            {
                ...
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::prepareRequestForSend removing route, no proxy, uri: %s\n", routeUri.data());
...
#ifdef TEST_PRINT
            osPrintf("SipTransaction::prepareRequestForSend using route address: %s\n", routeAddress.data());
#endif
        }
        ...
        if(toAddress.isNull())
        {
            ...
            //message.getUri(&toAddress, &port, &protocol);
#ifdef TEST_PRINT
         osPrintf("SipTransaction::prepareRequestForSend using URI address: %s protocol: %s\n", toAddress.data(), protocol.data());
#endif
        }
        ...
        {
#ifdef TEST_PRINT
            osPrintf("SipTransaction::prepareRequestForSend No URI address, using To address\n");
#endif
        ...
        else
        {
            ...
            //OsTime time;
            //OsDateTime::getCurTimeSinceBoot(time);
            //long beforeDnsSecs = time.seconds();
            // Look up DNS SRV records for sip servers.
            // Negative port means use it as the default, but don't insist on it.
            // Positive means insist on it, so don't use SRV.
            //userAgent.lookupSRVSipAddress(protocol, toAddress, port);
#ifdef TEST_PRINT
            //osPrintf("SipTransaction::doFirstSend Resolved DNS SRV server %s port %d (protocol %s)\n",toAddress.data(), port, protocol.data());
#endif
            //OsDateTime::getCurTimeSinceBoot(time);
            //long afterDnsSecs = time.seconds();
            //if(afterDnsSecs - beforeDnsSecs > 1)
            //{
            //    OsSysLog::add(FAC_SIP, PRI_WARNING, "SIP DNS SRV lookup for %s took %d seconds",
            //        toAddress.data(), (int)(afterDnsSecs - beforeDnsSecs));
            //}

            // set the DNS fields in the request
            //char buff[10];
            //sprintf( buff , "%d", port);
            //message.setDNSField(protocol , toAddress , buff);
        }
        //SDUA
                                      ...
                                      enum OsSocket::SocketProtocolTypes& toProtocol)
{
    ...
    if(toProtocol == OsSocket::UNKNOWN)
    {
        OsSysLog::add(FAC_SIP, PRI_DEBUG,
		      "SipTransaction::doFirstSend send protocol not "
...
#ifdef TEST_PRINT
        osPrintf("UA Sending SIP RESPONSE to: %s protocol: %d\n", toAddress.data(), toProtocol);
#endif
...
#ifdef TEST_PRINT
        osPrintf("SipTransaction::doFirstSend message send %s:%d\n", toAddress.data(), port);
#endif
    ...
    else
    {
        if(toProtocol != OsSocket::UDP)
        {
            ...
            OsSysLog::add(FAC_SIP, PRI_INFO,
                "SipTransaction::doFirstSend unknown protocol: %d using UDP",
                toProtocol);
        }
    ...
    else
    {
                ...
                mTransactionState == TRANSACTION_CALLING)
        {
            ...
            else
            {
                ...
                //                   SipMessageEvent::TRANSPORT_ERROR);
#ifdef TEST_PRINT
                osPrintf("SipTransaction::handleResendEvent sendOk: %d nextTimeout: %d\n",
                    sentOk, nextTimeout);
#endif
                if(!sentOk)
                {
                                        ...
                                        SipMessage*& delayedDispatchedMessage)
{
    ...
    if(outgoingMessage.isResponse())
    {
        OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::handleExpiresEvent %p expires event timed out on SIP response\n", this);
    }
...
#ifdef TEST_PRINT
        OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleExpiresEvent this: %p\n", this);
...
#       endif
        ...
        {
            // What is this????
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::handleChildIncoming invalid response code: %d\n",
                responseCode);
        }
...
#               endif
            ...
            {
                OsSysLog::add(FAC_SIP, PRI_DEBUG,
                    "SipTransaction::handleChildIncoming %d response with parent client transaction NOT dispatched",
                    responseCode);
            }
        }
        else
...
#               ifdef TEST_PRINT
                osPrintf( "SipTransaction::handleChildIncoming %p 3XX response - should not dispatch\n", this);
#               endif
                ...
                if(mIsServerTransaction)
                {
                    if(mpParentTransaction)
                    {
                        OsSysLog::add(FAC_SIP, PRI_ERR,
                                      "SipTransaction::handleChildIncoming 6XX server transaction is not top most parent\n");
                    }
...
#ifdef TEST_PRINT
            osPrintf( "SipTransaction::handleChildIncoming %p response=%d parent=%p final=%p dispatched=%d \n"
                     ,this, responseCode, mpParentTransaction,mpLastFinalResponse, mDispatchedFinalResponse
                     );
#endif
...
#               ifdef TEST_PRINT
                ...
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleChildIncoming\n%s",
                              dumpString.data());
#               else
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleChildIncoming %p\n", this );
#               endif

                if(mpParentTransaction)
                {
                    OsSysLog::add(FAC_SIP, PRI_ERR, "SipTransaction::handleChildIncoming %p server transaction is not top most parent\n", this);
                }
...
#                   endif
                ...
                else
                {
                    ...
                    if(findBestResponse(bestResponse))
                    {
                        ...
                        {
                            // Need a way to get the best message out to
                            // be dispatched.
                            if(delayedDispatchedMessage)
                            {
...
#                           ifdef TEST_PRINT
                            osPrintf("SipTransaction::handleChildIncoming %p should dispatch delayed message %d\n",
                                     this, __LINE__);
#                           endif
                            mDispatchedFinalResponse = TRUE;
                        }
           ...
           mRequestMethod.compareTo(SIP_INVITE_METHOD) == 0)
        {
                ...
                responseCode < SIP_3XX_CLASS_CODE)
            {
                ...
                if(incomingMessage.getViaField(&dummyVia, 1))
                {
                    ...
                    {
                         OsSysLog::add(FAC_SIP, PRI_DEBUG, 
                             "SipTransaction::handleChildIncoming proxy resending server transaction response %d\n",
                            responseCode);
                ...
                else
                {
                    ...
                    {
                        OsSysLog::add(FAC_SIP, PRI_WARNING, 
                            "SipTransaction::handleChildIncoming duplicate 2xx response received on UAC for INVITE with no ACK");
                    }
            ...
            else if(responseCode >= SIP_3XX_CLASS_CODE)
            {
                ...
                {
                    OsSysLog::add(FAC_SIP, PRI_WARNING, 
                        "SipTransaction::handleChildIncoming duplicate final error response received for INVITE with no ACK");
                }
            }
        }
    }
                                    ...
                                    SipMessage*& delayedDispatchedMessage)
{
    ...
    {
#ifdef LOG_FORKING
        OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleChildTimeoutEvent found top most parent: %p\n", this);
#endif
        ...
        //if(mIsServerTransaction)
        {
                    ...
                    )
            {
                ...
                else
                {
                    ...
                    {
#ifdef LOG_FORKING
                        OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleChildTimeoutEvent %p starting/still searching\n", this);
#endif
                    }
                    ...
                    else if(mpLastFinalResponse == NULL)
                    {
                        ...
                        {
#ifdef LOG_FORKING
                            OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleChildTimeoutEvent %p sending best response\n", this);
#endif
                            handleOutgoing(bestResponse,
                                            userAgent,
                                            transactionList,
                                           ...
                                           SipTransactionList& transactionList)
{
    ...
    while ((childTransaction = (SipTransaction*) iterator()))
    {
                ...
                )
        {
            ...
            {
#ifdef LOG_FORKING
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::startSequentialSearch %p failed to find child to transaction to pursue\n", this);
#endif
            }
        }
    }
                                   ...
                                   SipTransactionList& transactionList)
{
        ...
        mChildTransactions.isEmpty())
    {
        ...
        else if(mTransactionState < TRANSACTION_CONFIRMED)
        {
            ...
            if(mpDnsSrvRecords)
            {
                    ...
                    SipSrvLookup::isValidServerT(mpDnsSrvRecords[numSrvRecords]))
                {
                    ...
                    SipMessage::convertProtocolEnumToString(childTransaction->mSendToProtocol,
                        protoString);
                    osPrintf("Creating child DNS SRV transaction address: %s port: %d protocol: %s\n",
                    ...
                    if(childTransaction)
                    {
                        ...
                        // As DNS SRV is recursed serially the Q values are decrimented
                        // by a factor of the record index
                        childTransaction->mQvalue = mQvalue - numSrvRecords * 0.01;
            ...
            {
                OsSysLog::add(FAC_SIP, PRI_ERR, "SipTransaction::recurseDnsSrvChildren DNS server does not support DNS SRV");
            }
        }
    }
        ...
        mpRequest)
    {
             ...
             !childRecursing)
        {
                ...
                !childTransaction->mIsCanceled)
            {
                childRecursing = TRUE;
#ifdef LOG_FORKING
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::recurseDnsSrvChildren %p still pursing\n", this);
#endif
            }
            ...
            else
            {
                ...
                SipTransaction::getStateString(childTransaction->mTransactionState,
                    state);
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::recurseDnsSrvChildren %p transaction not recursed state: %s\n", this,
                    state.data());
#endif
            }
        }
...
#       endif
        ...
        while ((childTransaction = (SipTransaction*) iterator()))
        {
            ...
            if(childTransaction->mTransactionState == TRANSACTION_LOCALLY_INIITATED)
            {
...
#ifdef LOG_FORKING
                    OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::recurseChildren %p sending child transaction request\n", this);
#ifdef TEST_PRINT
                ...
                {
#ifdef LOG_FORKING
                    OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::recurseChildren %p nextQvalue: %f qDeltaSquare: %f\n", this,
                ...
                !childTransaction->mIsCanceled)
            {
                nextQvalue = childTransaction->mQvalue;
#ifdef LOG_FORKING
                OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::recurseChildren %p still pursing\n", this);
#ifdef TEST_PRINT
       ...
       else
       {
           ...
           if(protocol == OsSocket::TCP)
               sendOk = userAgent.sendTcp(&resendMessage,
                                ...
                                sendPort);
#ifdef TLS
           if(protocol == OsSocket::SSL_SOCKET)
                                         ...
                                         SipMessage*& delayedDispatchedMessage)
{
    if(delayedDispatchedMessage)
    {
        OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::handleIncoming delayedDispatchedMessage not NULL");
        delayedDispatchedMessage = NULL;
    }
    ...
    else if(relationship == MESSAGE_FINAL)
    {
        if(mpAck)
        {
            ...
            UtlString seqMethod;
            incomingMessage.getCSeqField(&cSeq, &seqMethod);
            OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::handleIncoming resending ACK for final response %s\n",
                seqMethod.data());
            ...
            OsSysLog::add(FAC_SIP, PRI_WARNING, "SipTransaction::handleIncoming received final response, sending existing ACK\n");
        }
...
#ifdef TEST_PRINT
    osPrintf( "SipTransaction::handleIncoming %p parent says shouldDispatch=%d delayed=%p\n", this, shouldDispatch, delayedDispatchedMessage );
#endif
    touch();
...
{
#ifdef TEST_PRINT
   OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::markBusy %p\n", this);
#endif
...
{
#ifdef TEST_PRINT
   OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::doMarkBusy(%d) %p\n", markData, this);
#endif
...
{
#ifdef TEST_PRINT
   OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipTransaction::markAvailable %p\n", this);
#endif
                                        ...
                                        if(mpLastFinalResponse)
                                        {
                                            ...
                                            else
                                            {
                                                if(!mIsUaTransaction)
                                                {
                                                    // Temporary fix for release 2.6 -- do not copy to release 2.7!
                                                    OsSysLog::add(FAC_SIP, PRI_INFO,
                                                                  "SipTransaction::messageRelationship ACK matches transaction with 2XX class response\n");
                                                }
                                                relationship = MESSAGE_2XX_ACK;
                                            }
                                        }
