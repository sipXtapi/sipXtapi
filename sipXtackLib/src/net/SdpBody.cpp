   ...
   if(bodyBytes)
   {
      ...
      do
      {
         ...
         if(nameFound)
         {
            ...
            //osPrintf("SdpBody::parseBody name: \"%s\" value: \"%s\"\n",
            //name.data(), value.data());
         }
         //else
         //{
         //    osPrintf("SdpBody::parseBody no name value found\n");
         //}
...
UtlBoolean SdpBody::getMediaPort(int mediaIndex, int* port) const
{
   ...
   osPrintf("SdpBody::getMediaPort port field: %s\n", portString.data());
                                 ...
                                 int* receiveCodecIndex) const
{
   ...
   *receiveCodecIndex = -1;
   ...
   osPrintf("SdpBody::getBestAudioCodecs numCodecs: %d\n", numRtpCodecs);
   while(mediaIndex >= 0 &&
         (!sendCodecFound || !receiveCodecFound))
   {
      mediaIndex = findMediaType(SDP_AUDIO_MEDIA_TYPE, mediaIndex);
      osPrintf("media index: %d\n", mediaIndex);
      if(mediaIndex >= 0)
      {
         getMediaPort(mediaIndex, rtpPort);
         osPrintf("Media port: %d\n", *rtpPort);
         if(*rtpPort >= 0)
         {
            osPrintf("SdpBody::getBestAudioCodecs media index: %d\n", mediaIndex);
            ...
            for(typeIndex = 0; typeIndex < numTypes; typeIndex++)
            {
               ...
               {
                  osPrintf("receive codec: %d send codec: %d\n receive index: %d send index %d\n",
                           payloadTypes[typeIndex], (rtpCodecs[codecIndex]).getCodecType(),
                           typeIndex, codecIndex);
   ...
   for(int typeIndex = 0; typeIndex < payloadIdCount; typeIndex++)
   {
...
#ifdef TEST_PRINT
            UtlString codecDump;
            codecsInCommonArray[numCodecsInCommon]->toString(codecDump);
            osPrintf("SdpBody::getBestAudioCodecs match by mime subtype:\n%s",
                     codecDump.data());
#endif
            numCodecsInCommon++;
              ...
              SdpCodec::SDP_CODEC_MAXIMUM_STATIC_CODEC)
      {
         if((matchingCodec = localRtpCodecs.getCodecByType(payloadTypes[typeIndex])))
         {
            ...
            UtlString codecDump;
            codecsInCommonArray[numCodecsInCommon]->toString(codecDump);
            osPrintf("SdpBody::getBestAudioCodecs match by static payload type:\n%s",
                     codecDump.data());
#endif
                             ...
                             const SdpBody* sdpRequest)
{
   ...
   osPrintf("SdpBody::addAudioCodecs rtpAddress: %s port: %d\n",
            rtpAddress, rtpPort);
   ...
   while(fieldFound)
   {
      ...
      if(fieldFound)
      {
         osPrintf("SdpBody::addAudioCodecs media index: %d\n", mediaIndex);
         ...
         {
            osPrintf("unsupported stuff mediaType: %s port: %d pairs: %d tranport type: %s\n",
                     mediaType.data(), mediaPort, mediaPortPairs, mediaTransportType.data());
                                              ...
                                              const char* fieldName)
{
   ...
   iter->reset();
   UtlContainable* nv = iter->findNext(&fieldNV);
