...
#define ASSERT_ARRAY_MESSAGE(message, expected, actual) \
  UrlSetFieldsTest::assertArrayMessage((expected),(actual), \
      CPPUNIT_SOURCELINE(), (message))

class UrlSetFieldsTest : public CppUnit::TestCase
{
    CPPUNIT_TEST_SUITE(UrlSetFieldsTest);
    CPPUNIT_TEST(testSetFields);
...
public:
    ...
    static void testParameterQuotingPutUser(Url& uri, const char* value)
    {
       uri.setUserId(value);
    }
    ...
    void testSetFields()
    {
       ...
       struct test_group {
          ...
          // The URI to modify by inserting the field.
          const char* start_uri_string;
          // The URI format version, either URI or contact-param.
          UtlBoolean uri_version;
          // sprintf format to generate the field string to be inserted.
          const char* field_format;
          // sprintf format to generate the expected value string.
          const char* expected_value_format;
          // Function to insert the field into the URI.
          // See the testParameterQuotingPut* helper functions
          // defined above.
          void (*set_value)(Url& uri, const char* value);
       };
...
// Also, these tests don't account for the feature tag syntax.
            ...
            test_group++)
       {
          ...
          for (unsigned int i = 0; i < number_of_chars; i++)
          {
             ...
             Url uri(test_groups[test_group].start_uri_string,
                     test_groups[test_group].uri_version);
             // Construct the value to put.
             char field_value[100];
             sprintf(field_value, test_groups[test_group].field_format,
                     character_to_test);
             ...
             sprintf(expected_value, test_groups[test_group].expected_value_format,
                     character_to_test);
             // Modify the URI.
             test_groups[test_group].set_value(uri, field_value);
             // Extract the actual value.
             UtlString actual_value;
             if (test_groups[test_group].uri_version == STRING_URI)
             {
                uri.getUri(actual_value);
             }
             else
             {
                uri.toString(actual_value);
             }
             ...
             sprintf(msg, "Testing char '%c' %s in %s format: '%s'",
                     ...
                     test_groups[test_group].start_uri_string);
//printf("%s\n", msg);
...
CPPUNIT_TEST_SUITE_REGISTRATION(UrlSetFieldsTest);
