...
void initSysLog(OsConfigDb* pConfig)
{
   ...
   OsSysLog::add(FAC_SIP, PRI_INFO, "%s : %s", SIPX_SAFENULL(CONFIG_SETTING_LOG_CONSOLE), bConsoleLoggingEnabled ? "ENABLE" : "DISABLE") ;
...
main(int argc, char* argv[] )
{
   ...
   for(int argIndex = 1; argIndex < argc; argIndex++)
   {
      ...
      //{
      //   osPrintf("Version: %s (%s)\n", SIPX_SAFENULL(SIPXPBX_VERSION), SIPX_SAFENULL(SIPXPBX_VERSION_COMMENT));
      //   return(1);
      //}
      //else 
