...
OsStatus MonitoredProcess::check()
{
    ...
    if (processstate != PROCESS_STOPPED && processstate != PROCESS_STOPPING)
    {
        ...
        if ( processstate != PROCESS_FAILED && processstate != PROCESS_STOPPED &&
             processstate != PROCESS_STOPPED && processstate != PROCESS_STOPPING && !mbRestartEnabled )
        {
            mbRestartEnabled = TRUE;
            mnTotalRestarts = 0;
        }
...
*/
        ...
        if ( processstate == PROCESS_NEVERRUN || bIsGhost )
        {
