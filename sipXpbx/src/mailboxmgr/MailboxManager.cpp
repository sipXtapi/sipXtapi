           ...
           result = fi.findNext(entry))
      {
         ...
         if (!message_rejected && sort_criterion != SORT_OTHER)
         {
            ...
            case SORT_INBOX:
               ...
               // the list.
               priority = !unheard;
               // We do not want to display all the heard messages in next block
               if (!unheard && nextBlockHandle > 0)
               {
                  message_rejected = !unheard;
               }
               break;
    ...
    const UtlString& loginPassToken )
{
   ...
   // Fill in the real code after we have decided how we should change the password in IMDB.
