    ...
    const UtlString& loginPassToken )
{
   ...
   // Create the MD5 Digest for loginPassToken
   UtlString newToken;
   UtlString textToEncode = userId + ":" + m_defaultRealm + ":" + loginPassToken;
   NetMd5Codec::encode( textToEncode, newToken );

   OsSysLog::add(FAC_MEDIASERVER_CGI, PRI_DEBUG,
                 "Sending the new pin to Config server for user %s\n", userId.data());

   // Send to the config server
   Url url(m_configServerSecureUrl + "//pds/soap/services/urn:UserService");
   ...
   sprintf(buffer, _SOAP_MSG_TEMPLATE_,
           ...
           "deleteUser", "urn:UserService", "abcd", "deleteUser");
   ...
   pRequest->addHeaderField("Authorization", "Basic c3VwZXJhZG1pbjoxMjM0");
   ...
   pRequest->setBody(body);
   pRequest->setContentType("text/xml; charset=utf-8");
   pRequest->setContentLength(soapMessage.length());
   // @TODO@ modify the action
   pRequest->addHeaderField("SOAPAction", "#deleteUser");
   ...
   pResponse->get(url, *pRequest, 20*1000);
