    ...
    const int& iTotalCount ) const
{
    ...
    UtlString strUnheardCount = temp;

    sprintf ( temp, "%d", totalCount );
    UtlString strTotalCount = temp;

    // Check the filesystem to make sure that we are writing correct values.
    int substrLength = summaryFileLocation.index( FOLDER_SUMMARY_FILE ) - 1;
    UtlString summaryFilePath = summaryFileLocation( 0, substrLength );
    OsPath path( summaryFilePath );
    OsFileIterator fi( path );
    OsPath entry;
    int iActualUnheardCount = 0;
    int iActualTotalCount = 0;
    ...
    if( (unheardCount == iActualUnheardCount) && (totalCount == iActualTotalCount) )
    {
        // Go ahead and update the summary file.
        TiXmlDocument doc ( summaryFileLocation );
        ...
        if( doc.LoadFile() )
        {
            ...
            if ( rootNode != NULL )
            {
                TiXmlNode* node = rootNode->FirstChild("unheard-msg-count");
                if ( (node != NULL) && (node->Type() == TiXmlNode::ELEMENT) )
                {
                    if ( node->FirstChild() && node->FirstChild()->Type() == TiXmlNode::TEXT )
                    {
                        node->FirstChild()->SetValue( strUnheardCount );
                    }
                }

                node = rootNode->FirstChild("total-msg-count");
                if ( (node != NULL) && (node->Type() == TiXmlNode::ELEMENT) )
                {
                    if ( node->FirstChild() && node->FirstChild()->Type() == TiXmlNode::TEXT )
                    {
                        node->FirstChild()->SetValue( strTotalCount );
                    }
                }

                // Save the updated document
                if ( doc.SaveFile( summaryFileLocation ) == true )
                {
                    result = OS_SUCCESS;
                }
            }
    ...
    } else
    {
        writeToLog( "UpdateMailboxFolderSummary",
            "Contents of summary file DO NOT match the filesystem - " + summaryFileLocation,
            PRI_INFO);

        writeToLog( "UpdateMailboxFolderSummary",
                    "Fixing the summary file ...",
                    PRI_INFO);

        sprintf ( temp, "%d", iActualUnheardCount );
        strUnheardCount = temp;

        sprintf ( temp, "%d", iActualTotalCount );
        strTotalCount = temp;

        OsFile summaryfile( summaryFileLocation );
        summaryfile.remove();
    ...
    if ( summaryFileLocation.index( m_inboxFolder )  != UTL_NOT_FOUND )
    {
	        ...
	        OsSysLog::add(FAC_MEDIASERVER_CGI, PRI_DEBUG,
		        ...
		        OsSysLog::add(FAC_MEDIASERVER_CGI, PRI_DEBUG,
                        ...
                        } else // Successful match
                        {
                            ...
                            CredentialDB::getInstance()->getCredential (
                                   mailboxUrl,                  // IN
                                   m_defaultRealm,              // IN
                    ...
                    } else  // mailboxUrl is now valid
                    {
                        ...
                        CredentialDB::getInstance()->getCredential (
                            ...
                            dbAuthType );                // OUT
                        OsSysLog::add(FAC_MEDIASERVER_CGI, PRI_DEBUG,
                                      "MailboxManager::validateMailbox: CredentialDB::...getCredential called, mailboxUrl = '%s', m_defaultRealm = '%s', userOrExtensionAtOptDomain = '%s', dbPassToken = '%s', dbAuthType = '%s'",
                                      mailboxUrl.toString().data(),
                                      m_defaultRealm.data(),
                                      userOrExtensionAtOptDomain.data(),
                ...
                } else
                {
                    ...
                    // update the userOrExtensionAtOptDomain
                    if (!CredentialDB::getInstance()->getCredential (
                            mailboxUrl,                  // IN
                            m_defaultRealm,              // IN
    ...
    const UtlString& messageids )
{
   ...
   if ( result == OS_SUCCESS )
   {
      ...
      if ( messageids == UPDATE_ALL_MSG_STATES )
      {
         ...
         result = fi.findFirst(entry, "[0-9]*.sta", OsFileIterator::FILES);
      ...
      {
         // Indicates that all messages with ids <= messageid should be updated.
         // Used by the TUI.
      ...
      {
         logContents = "Messageids for updating status is to '0'. Nothing to update!";
      ...
      else
      {
         ...
         while (nameValuePair.next(id, MESSAGE_DELIMITER))
         {
            ...
            if (result != OS_SUCCESS)
            {
               msgCount--;
               errorCount++;
               logContents = "message " + id + " does not exist in " + mailboxPath; 
               writeToLog( "UpdateMessageStates", logContents, PRI_DEBUG );
            }
      ...
      if (errorCount > 0)
      {
         logContents = "Could not find one or more messages from list '" +
            messageids + "' in " + category;
         result = OS_NOT_FOUND;
      }
   } else
   {
      logContents = "Failed to obtained the mailbox path for " + mailboxIdentity;
   }

   if( result != OS_SUCCESS )
      writeToLog( "UpdateMessageStates", logContents, PRI_ERR );
                                        ...
                                        const UtlString& messageIds )
{
    ...
    if( result == OS_SUCCESS )
    {
        ...
        UtlString summaryFileLocation = mailboxPath + OsPathBase::separator + FOLDER_SUMMARY_FILE ;
        ...
        while ( result == OS_SUCCESS )
        {
            ...
            if( result == OS_FILE_NOT_FOUND )
                result = createSummaryFile(summaryFileLocation, "0", "0" );
            else
                result = OS_FAILED ;
        }
        else
        {
            ...
            if( result == OS_FILE_NOT_FOUND )
                result = updateMailboxFolderSummary( mailbox, summaryFileLocation, unheardmsgcount, totalmsgcount );
            else
                result = OS_FAILED ;
        }
    }
    ...
    UtlString& rMediaserverUrl ) const
{
    ...
    else
    {
        ...
        // Bug XVX-70: m_mediaserverUrl is read from voicemail.xml.in
        // and its normal value is 'http://localhost... ' so this
        // will not match if incoming request is https.  Above logic
        // that matches incoming protocol seems valid so I do not know
        // how why this check was put in here. I leave it hear in case
        // there's an event flow that I was not aware of, but refrain
        // from returning and error -- DLH 10/28/04
        if ( m_mediaserverUrl.first( protocol.data() ) == UTL_NOT_FOUND )
        {
            writeToLog( "getMediaserverURLForWeb", "Protocol Mismatch", PRI_ERR );
            OsSysLog::add(FAC_MEDIASERVER_CGI, PRI_ERR, 
