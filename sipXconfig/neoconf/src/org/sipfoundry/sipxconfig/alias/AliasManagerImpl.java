 ...
 */
public class AliasManagerImpl implements AliasManager, ApplicationListener {
    ...
    /**
     * Return the objects with this alias by asking all the AliasOwners.
    ...
    public Collection getObjectsWithAlias(String alias) {
        ...
        for (Iterator iter = getAliasOwners().iterator(); iter.hasNext();) {
            AliasOwner owner = (AliasOwner) iter.next();
            objects.add(owner.getObjectsWithAlias(alias));
        }
        return objects;
    }
    ...
    protected Collection getAliasOwners() {
        if (m_aliasOwners == null) {    // lazy initialization
            ...
            for (Iterator i = beanMap.values().iterator(); i.hasNext();) {
                ...
                if (owner instanceof Proxy && !(owner instanceof AliasManagerImpl)) {
