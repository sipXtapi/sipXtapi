...
OsStatus record32(int timeMS, const char* micName = NULL,
   const char* echoName = NULL, const char* spkrName = NULL,
   const char* mic32Name = NULL, const char* spkr32Name = NULL,
   const char* echoIn8Name = NULL, const char* echoIn32Name = NULL,
   const char* playName = NULL, int toneOptions = 0,
   int repeat = 0, OsNotification* completion = NULL);

OsStatus startRecording32(const char* audioFileName, UtlBoolean repeat,
...
#ifdef INJECT_32K_TO_SPEAKER /* [ */
   UtlBoolean handleStartSpkr(MpFlowGraphMsg& rMsg);
     //:Handle the FLOWGRAPH_START_PLAY message for a speaker file.
     // Returns TRUE if the message was handled, otherwise FALSE.
#endif /* INJECT_32K_TO_SPEAKER ] */
