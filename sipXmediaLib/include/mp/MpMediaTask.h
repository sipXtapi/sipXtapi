...
#ifdef _VXWORKS /* [ */
   enum { // media task execution priority
          DEF_MEDIA_TASK_PRIO_NORMAL = 9,   // Normal, default
          DEF_MEDIA_TASK_PRIO_LOW = 80,     // Throttled back
          DEF_MEDIA_TASK_PRIO_TROUBLE = 250 // Serious overload
          };

#define MEDIA_TASK_PRIO_NORMAL media_task_prio_normal
#define MEDIA_TASK_PRIO_LOW media_task_prio_low
#define MEDIA_TASK_PRIO_TROUBLE media_task_prio_trouble

   static int media_task_prio_normal;
   static int media_task_prio_low;
   static int media_task_prio_trouble;
#endif /* _VXWORKS ] */
...
/* ============================ MANIPULATORS ============================== */
   ...
   static OsStatus signalFrameStart(void);
     ...
     // to send the signal.
#ifdef _VXWORKS /* [ */
   static int showHistogram(void);
   static int setHistInterval(int frames);
#endif /* _VXWORKS ] */
...
#ifdef _VXWORKS /* [ */
   static int tweakPrioRange(int normal, int low, int lowest);
#endif /* _VXWORKS ] */
...
private:
   ...
   unsigned  mStartTicks;   // Start time (in ticks) for the current frame 
                            //  processing interval
   unsigned  mStopTicks;    // Stop time (in ticks) for the current frame
                            //  processing interval
   int       mLimitTicks;   // Frame processing time limit (in ticks)
...
#ifdef _VXWORKS /* [ */
   static unsigned int sSignalTicks;  // Time (in ticks) for the current
                                      //  frame start signal
   static unsigned int sMinTicks;
   static unsigned int sMaxTicks;

   static int       sHistogram[20];
   static int       sHistFrames;
   static int       sMaxHistFrames;
#endif /* _VXWORKS ] */
   ...
   UtlBoolean handleWaitForSignal(void);
