   ...
   if (pSrc != NULL)
   {
      ...
      while (!mbEnd && nextDataChunk(iDataLength))
      {
         ...
         while ((iDataLength > 0) && !mbEnd)
         {
            ...
            {
                osPrintf("Unsupport bit per sample rate!\n");
            }
...
UtlBoolean StreamWAVFormatDecoder::nextDataChunk(int& iLength)
{
   ...
   if (pDataSource != NULL)
   {      
      while (!mbEnd && pDataSource->read((char*) Header, sizeof(Header), iRead) == OS_SUCCESS)
      {      
         ...
         if (iCurrentPosition == 4 && memcmp(Header, "RIFF", 4) != 0)
         {
             ...
             mbEnd = TRUE;
	     // For release 2.6, this message is downgraded to PRI_INFO, to
	     // keep the logs from getting full.
	     // The new Media Player in 2.7 does not generate these messages,
	     // so this change should not be propagated to 2.7.
             syslog(FAC_STREAMING, PRI_INFO, "StreamWAVFormatDecoder::nextDataChunk (RIFF not detected. Exiting!)\n");
             break;
         }
         else
         if (memcmp(Header, "RIFF", 4) == 0)
         {
            ...
            if (pDataSource->read((char*) &blockSize, sizeof(blockSize), iRead) != OS_SUCCESS)
            {
                ...
                osPrintf("Error reading block size of RIFF\n");
                break;
            }
         ...
         if (memcmp(Header, "fmt ", 4) == 0)
         {
            ...
            if (pDataSource->read((char*) &blockSize, sizeof(blockSize), iRead) == OS_SUCCESS)
            {
                ...
                if (pDataSource->read((char*) &formatChunkInfo, sizeof(formatChunkInfo), iRead) == OS_SUCCESS)
                {
                        ...
                        formatChunkInfo.nChannels != 1)
                    {
                        syslog(FAC_STREAMING, PRI_ERR, "StreamWAVFormatDecoder::nextDataChunk (File is not 16 bit, 8khz, mono, signed format!)\n");
                         osPrintf("******************************************************\n");
                         osPrintf("ERROR: Streaming from http source can not occur.\n");
                         osPrintf("       File is not 16 bit, 8khz, mono, signed format!\n");
                         osPrintf("******************************************************\n");
                ...
                else
                {
                    ...
                    osPrintf("Error reading block fmt\n");
                    break;
                }
            }
            else
            {
                ...
                osPrintf("Error reading block size of fmt chunk\n");
                break;
            }
         ...
         else
         {
            ...
            else
            {
                ...
                osPrintf("Error reading block size of block: %s\n",Header);
                break;
            }
         }
      }
