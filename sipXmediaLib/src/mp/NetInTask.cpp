...
OsBSem     NetInTask::sLock(OsBSem::Q_PRIORITY, OsBSem::FULL);
...
int NetInTaskHelper::run(void* pInst)
{
    ...
    mpNotify->signal(0);
    osPrintf("NetInTaskHelper::run()... returning 0, after %d tries\n", (1001-trying));
...
int NetInTask::getWriteFD()
{
    ...
    if (OsTask::getCurrentTask() == NetInTask::spInstance) {
        ...
        osPrintf("NetInTask: launching connection helper\n");
        pNotify = new OsEvent;
