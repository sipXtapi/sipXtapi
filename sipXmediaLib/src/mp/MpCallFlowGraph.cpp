...
#if defined(WIN32) || defined(__pingtel_on_posix__) /* [ */
...
OsStatus MpCallFlowGraph::record32(int ms, const char* micName,
   const char* echoOutName, const char* spkrName,
   const char* mic32Name, const char* spkr32Name,
   const char* echoIn8Name, const char* echoIn32Name,
   const char* playName, int toneOptions,
   int repeat, OsNotification* completion)
{
   if (NULL == this) {
      MpMediaTask* pMT = MpMediaTask::getMediaTask(0);
      MpCallFlowGraph* pIF = (MpCallFlowGraph*) pMT->getFocus();
      if (NULL != pIF) return pIF->record32(ms, micName, echoOutName, spkrName,
         mic32Name, spkr32Name, echoIn8Name, echoIn32Name, playName,
         toneOptions, repeat, completion);
      return OS_INVALID;
   }

   if (NULL != micName) {
      setupRecorder(RECORDER_MIC, micName, ms, 9999, completion);
   }
   if (NULL != echoOutName) {
      setupRecorder(RECORDER_ECHO_OUT, echoOutName, ms, 9999, completion);
   }
   if (NULL != spkrName) {
      setupRecorder(RECORDER_SPKR, spkrName, ms, 9999, completion);
   }
   if (NULL != mic32Name) {
      setupRecorder(RECORDER_MIC32K, mic32Name, ms, 9999, completion);
   }
   if (NULL != spkr32Name) {
      setupRecorder(RECORDER_SPKR32K, spkr32Name, ms, 9999, completion);
   }
   if (NULL != echoIn8Name) {
      setupRecorder(RECORDER_ECHO_IN8, echoIn8Name, ms, 9999, completion);
   }
   if (NULL != echoIn32Name) {
      setupRecorder(RECORDER_ECHO_IN32, echoIn32Name, ms, 9999, completion);
   }
   return startRecording32(playName, repeat, completion);
}

OsStatus MpCallFlowGraph::startRecording32(const char* audioFileName,
                        UtlBoolean repeat, OsNotification* event)
{
   OsStatus  res = OS_SUCCESS;
   MpFlowGraphMsg msg(MpFlowGraphMsg::FLOWGRAPH_START_RECORD, NULL,
                   NULL, NULL, 0, START_PLAY_NONE);

   if (NULL != audioFileName) {
      res = mpToSpkr->playFile(audioFileName, repeat, event);
      if (res == OS_SUCCESS) {
         msg.setInt2(START_PLAY_SPKR);
      }
   }
...
#ifdef INJECT_32K_TO_SPEAKER /* [ */
UtlBoolean MpCallFlowGraph::handleStartSpkr(MpFlowGraphMsg& rMsg)
{
   mpToSpkr->startPlay();
   return TRUE;
}
...
#ifdef INJECT_32K_TO_SPEAKER /* [ */
   if (START_PLAY_SPKR == startPlayer) mpToSpkr->startPlay();
#endif /* INJECT_32K_TO_SPEAKER ] */
   for (i=0; i<MAX_RECORDERS; i++) {
      if (NULL != mpRecorders[i]) {
         mpRecorders[i]->begin();
