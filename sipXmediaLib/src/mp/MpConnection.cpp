...
void MpConnection::addPayloadType(int payloadType, MpDecoderBase* decoder)
{
   ...
   assert((0 <= payloadType) && (payloadType < NUM_PAYLOAD_TYPES));
#if 1 /* [ */
   //assert(NULL == mpPayloadMap[payloadType]);
   if (!(NULL == mpPayloadMap[payloadType])) {
      osPrintf("MpConn::addPT [would have] failed"
         " assert(NULL == mpPayloadMap[payloadType])\n");
   }
#endif /* ] */
