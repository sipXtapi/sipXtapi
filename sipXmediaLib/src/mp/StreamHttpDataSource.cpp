...
/*
   ...
   if (now != seconds)
   {
      ...
      if (now % 5 == 0)
         osPrintf("KB/S: %.03f (l=%.03f, h=%.03f, a=%.03f), queued=%.03f\n", kbps, low, hi, ave, (float) pSource->getBufferedLength()/1024.0) ;
      seconds = now ;
      bytes = 0 ;
   }
*/
