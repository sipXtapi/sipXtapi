...
if (HandAEC == 0)
{
...
#endif /* _VXWORKS ] */
    {       /* If it is the handset case */
         out = in8;                     /* pass the input to output */
         inBufs[0] = NULL;
         if (NULL == out) {
             out = MpBuf_getFgSilence();
         }
...
#ifdef DEBUG_TC /* [ */
         if (doShowMe())  {
            osPrintf("MprES::dPF:  mpPrev==NULL\n");
         }
#endif /* DEBUG_TC ] */
...
#ifdef DEBUG_TC /* [ */
         if (doShowMe())  {
            osPrintf("MprES::dPF: downsampling from 0x%X to 0x%X\n",
               (int) mpPrev, (int) out2);
         }
#endif /* DEBUG_TC ] */
...

   {
      ...
      if ( iSub == 0)
      {
         if (MpCodec_isBaseSpeakerOn()) //speakerphone
         {
