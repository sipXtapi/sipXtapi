...
#ifdef _VXWORKS /* [ */
#include "dev/sa1100/DevTimers.h"
...
#ifdef _VXWORKS /* [ */
   int MpMediaTask::media_task_prio_normal =
                        MpMediaTask::DEF_MEDIA_TASK_PRIO_NORMAL;
   int MpMediaTask::media_task_prio_low =
                        MpMediaTask::DEF_MEDIA_TASK_PRIO_LOW;
   int MpMediaTask::media_task_prio_trouble =
                        MpMediaTask::DEF_MEDIA_TASK_PRIO_TROUBLE;

   unsigned int MpMediaTask::sSignalTicks;
   unsigned int MpMediaTask::sMinTicks;
   unsigned int MpMediaTask::sMaxTicks;
   int       MpMediaTask::sHistogram[20];
   int       MpMediaTask::sHistFrames;
   int       MpMediaTask::sMaxHistFrames = 2160000; // 6 hours
#endif /* _VXWORKS ] */
...
{
#ifdef _VXWORKS
   mLimitTicks = DevTimers_usecsToTicks(usecs);
#endif
   mLimitUsecs = usecs;
...
#ifdef _VXWORKS /* [ */
         if (OS_SUCCESS == ret) {
            sSignalTicks = DevTimers_getTicks(); // record the time
         }
...
int MpMediaTask::showHistogram(void)
{
   ...
   mini = ((sMinTicks + (3686/2)) * 100) / 3686;
   minf = mini % 100;
   mini = mini / 100;

   maxi = ((sMaxTicks + (3686/2)) * 100) / 3686;
   maxf = maxi % 100;
   maxi = maxi / 100;
   ...
   osPrintf("Histogram (%d frames, range:%d.%02d %d.%02d): %s\n",
...
#ifdef _VXWORKS /* [ */
// (static) Adjust priority "constants"
int MpMediaTask::tweakPrioRange(int normal, int low, int lowest)
{
   if (0 > normal) {
      media_task_prio_normal = DEF_MEDIA_TASK_PRIO_NORMAL;
      media_task_prio_low = DEF_MEDIA_TASK_PRIO_LOW;
      media_task_prio_trouble = DEF_MEDIA_TASK_PRIO_TROUBLE;
      return 0;
   }
   if (0 == normal) {
      osPrintf("Current MediaTask priorities: %d, %d, %d\n",
         media_task_prio_normal,
         media_task_prio_low,
         media_task_prio_trouble);
      return 0;
   }
   if ((0 < normal) && (256 > normal)) {
      MpMediaTask::media_task_prio_normal = normal;
   }
   normal = MpMediaTask::media_task_prio_normal;
   if ((0 < low) && (256 > low)) {
      if (normal > low) low = normal;
      media_task_prio_low = low;
      media_task_prio_trouble = lowest;
   }
   low = MpMediaTask::media_task_prio_low;
   if ((0 < lowest) && (256 > lowest)) {
...
#ifdef _VXWORKS /* [ */
#define SPKR_STATE_DEBOUNCE 20 // # of consecutive media frames to debounce
   static UtlBoolean lastExtSpkrState = !MpCodec_isExtSpkrPresent();
   static int countSpkrStateChanges = SPKR_STATE_DEBOUNCE;
#endif /* _VXWORKS ] */
...
#ifdef _VXWORKS
      mStartTicks = DevTimers_getTicks(); // record the processing start time
#endif
 ...
 ///////////////////////////////////////////////////////////////////
...
#ifdef _VXWORKS
   mStopTicks = DevTimers_getTicks();  // record the processing stop time
   ...
   i = (mStopTicks - sSignalTicks);
   if (i < sMinTicks) sMinTicks = i;
   if (i > sMaxTicks) sMaxTicks = i;
   i = (i + (3686/2)) / 3686;
   if (i < 1) i = 0;
   if (i > 18) i = 19;
   sHistogram[i]++;
...
#ifdef _VXWORKS /* [ */
   // Poll the external speaker jack detect bit
   if (MpCodec_isExtSpkrPresent() == lastExtSpkrState) {
      countSpkrStateChanges = 0;
   } else {
      if (SPKR_STATE_DEBOUNCE < countSpkrStateChanges++) {
         countSpkrStateChanges = 0;
         lastExtSpkrState = !lastExtSpkrState;
      // (PsPhoneTask::getPhoneTask())->extSpeakerConnect(lastExtSpkrState);
         osPrintf("External speaker %s.\n",
            lastExtSpkrState ? "inserted" : "removed");
      }
   }
#endif /* _VXWORKS ] */
