   ...
   SipRedirectorPrivateStorage*& privateStorage)
{
            ...
            userId.compareTo(ALL_CREDENTIALS_USER) == 0)
   {
      ...
      else
      {
         return RedirectPlugin::LOOKUP_ERROR_REQUEST;
            ...
            mOrbitFileReader.findInOrbitList(orbitName) != NULL)
   {
            ...
            userId.index(mCallRetrieveCode.data()) == 0)
   {
      ...
      {      
         ...
         if (mOrbitFileReader.findInOrbitList(orbitName) != NULL)
         {
            return lookUpDialog(requestString,
                                response,
                                requestSeqNo,
   ...
   State stateFilter)
{
   ...
   else
   {
      ...
      requestUri.getUserId(userId);
      requestUri.getHostAddress(domain);
      OsSysLog::add(FAC_SIP, PRI_DEBUG, "SipRedirectorPickUp::lookUpDialog userId '%s' domain '%s'",
          userId.data(), domain.data());
