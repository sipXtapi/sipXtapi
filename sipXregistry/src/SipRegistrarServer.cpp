    ...
    const UtlString& realm)
{
    ...
    mRegisterHooks = sipRegisterHooks;
        ...
        if ( REGISTER_SUCCESS == returnStatus )
        {
            ...
            else
            {
                ...
                if ( REGISTER_SUCCESS == returnStatus )
                {
                    ...
                    mRegisterHooks->doHooks(registerMessage, longestExpiration, mSipUserAgent);
                }
            }
        }
    }
    ...
    SipMessage& responseMessage )
{
    ...
    else
    {
        ...
        {
            OsSysLog::add( FAC_AUTH, PRI_DEBUG, "SipRegistrar::isAuthorized() - Message Authorization received: "
                                          ...
                                          reqUri, mRealm, mNonceExpiration))
                {
                                                                   ...
                                                                   ))
                    {
                      ...
                      if ((isAuthorized = message.verifyMd5Authorization(requestUser.data(),
                        ...
                        {
                          OsSysLog::add(FAC_AUTH, PRI_DEBUG, "SipRegistrar::isAuthorized() response auth hash matches\n");
                        }
                      else
                        {
                          OsSysLog::add(FAC_AUTH, PRI_DEBUG, "SipRegistrar::isAuthorized() response auth hash does not match\n");
                        }
                    }
                    else // failed to get credentials
                    {
                        OsSysLog::add(FAC_AUTH, PRI_INFO, "Unable to get credentials for '%s'\nrealm='%s'\nuser='%s'",
                                toUri.data(), mRealm.data(), requestUser.data()) ;
                    }
                }
                else // nonce is not valid
                {
                    syslog(FAC_AUTH, PRI_INFO, "Invalid nonce for '%s'\nnonce='%s'\ncallId='%s'\nreqUri='%s'",
