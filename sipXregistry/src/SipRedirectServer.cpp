    ...
    SipMessage& response)
{
    ...
    if ( numUnexpiredContacts > 0 )
    {
		...
		// Temporary fix for 2.6 -- lower logging level to INFO.
                OsSysLog::add( FAC_SIP, PRI_INFO, "SipRedirectServer::registrationDBLookUp() - "
                              "URI MAP '%s' to '%s'\n",
        ...
        for ( int i=0; i<numAliasContacts; i++ )
        {
            ...
            {   // should be a valid contact, set mailboxUrl to it and
                ...
                if ( !contactUri.isUserHostPortEqual(requestUri) )
                {
                    ...
                    // Temporary fix for 2.6 -- lower logging level to INFO.
                    OsSysLog::add( FAC_SIP, PRI_INFO, "SipRedirectServer::aliasDBLookUp() - "
                                  "URI MAP '%s' to '%s'\n",
        ...
        if ( numUrlMappingRegistrations > 0 )
        {
            ...
            for ( int i=0 ; i<numUrlMappingRegistrations; i++ )
            {
                ...
                if ( !contactUri.isUserHostPortEqual(requestUri) )
                {
                    ...
                    // Temporary fix for 2.6 -- lower logging level to INFO.
                    OsSysLog::add(FAC_SIP, PRI_INFO, "SipRedirectServer::urlMappingDBLookUp(%s) - "
                                  "URI MAP '%s' to '%s'\n",
