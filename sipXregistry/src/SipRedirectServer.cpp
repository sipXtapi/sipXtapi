   ...
   const char* configFileName)
{
   ...
   // :TODO: Remove these when testing is done!
   mRedirectors[6] = new SipRedirectorTest;
   mRedirectors[6]->initialize(configParameters, configDb, mpSipUserAgent, 6);

   mRedirectors[7] = new SipRedirectorTest;
   mRedirectors[7]->initialize(configParameters, configDb, mpSipUserAgent, 7);
   ...
   mRedirectors[8] = new SipRedirectorPickUp;
   mRedirectors[8]->initialize(configParameters, configDb, mpSipUserAgent, 8);

// Update this test with the index of the last redirector loaded.
#if 8 != (MREDIRECTORCOUNT-1)
                                        ...
                                        RedirectSuspend* suspendObject)
{
   ...
   const Url requestUri(stringUri);
   ...
   for (int i = 0; i < MREDIRECTORCOUNT; i++)
   {
      ...
      case SipRedirector::LOOKUP_ERROR_REQUEST:
         ...
         // should end immediately and a 404 response returned.
