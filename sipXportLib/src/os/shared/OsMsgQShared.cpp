                           ...
                           const UtlBoolean sendFromISR)
{
   ...
   else
   {
      ...
      if (increasedLevel)
      {
         // Report increasing the message queue at INFO level, unless
         // we have increased to its maximum (which might indicate
         // that it is nearing capacity), which is reported at WARNING
         // level.
         // As a special hack for release 2.6, if mMaxMsgs is 1600,
         // report at INFO level, because this queue is intended to fill
         // and its source knows how to deal with that.
         OsSysLog::add(FAC_KERNEL,
                       (mMaxMsgs == 1600 ? PRI_INFO :
                        (curCount == mMaxMsgs ? PRI_WARNING : PRI_INFO)),
