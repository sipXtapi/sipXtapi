    ...
    mSSL(NULL)
{
   if (mIsConnected)
   {
      ...
      OsSysLog::add(FAC_KERNEL, PRI_DEBUG, 
                    "OsSSLConnectionSocket::_ (socket, timeout)");
   }
   else
   {
      OsSysLog::add(FAC_KERNEL, PRI_ERR, 
                    "OsSSLConnectionSocket::~ underlying OsConnectionSocket not connected!!");
   }
}
    ...
    OsConnectionSocket(connectedSocketDescriptor)
{
    ...
    OsSysLog::add(FAC_KERNEL, PRI_DEBUG, 
                  "OsSSLConnectionSocket::~ (SSL, socket)");
}
       ...
       if (mSSL && (socketDescriptor > OS_INVALID_SOCKET_DESCRIPTOR))
       {
          ...
          else
          {
             OsSSL::logError(FAC_KERNEL, PRI_ERR,
                             "OsSSLConnectionSocket SSL_connect failed", err);
             mIsConnected = FALSE;
             OsConnectionSocket::close();
