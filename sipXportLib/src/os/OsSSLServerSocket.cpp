...
#ifdef _VXWORKS
    #define PUBCERTFILE "/flash0/public.crt"
    #define PRIVKEYFILE "/flash0/server.key"
#else
    #define PUBCERTFILE "server-cert.pem"
    #define PRIVKEYFILE "server-priv-key.pem"
#endif
...
int OsSSLServerSocket::pemPasswdCallbackFunc(char *buf, int size, int rwflag, void *userdata)
{
    if (sPrivateKeyPassword.length() <= (size_t)size)
        strcpy(buf,(char *)sPrivateKeyPassword.data());

    return sPrivateKeyPassword.length();
}
...
OsConnectionSocket* OsSSLServerSocket::accept()
{
   ...
   {
	OsSysLog::add(FAC_KERNEL, PRI_ERR, "OsSSLServerSocket: accept exiting because socketDescriptor is %d\n",socketDescriptor);
	return NULL;
   }

   int err;    
   OsSSLConnectionSocket* connectSock = NULL;
   ...
   if (clientSocket < 0)
   {
	...
	SSL*     pSSL = SSL_new (mCTX);
        ...
        if (pSSL) 
        {
            ...
            connectSock = new OsSSLConnectionSocket(pSSL,clientSocket);

            err = SSL_accept (pSSL);
            
            if (connectSock)
                OsSysLog::add(FAC_KERNEL, PRI_INFO, "OsSSLServerSocket::accept - SUCCESS creating SSL socket for accept.\n");
        }
        else
        {
            OsSysLog::add(FAC_KERNEL, PRI_ERR, "Error creating new SSL object in SSL server accept method.\n");
            socketDescriptor = OS_INVALID_SOCKET_DESCRIPTOR;
        }
#endif
...
//        SSL_load_error_strings();
	...
	// disable server connection caching
        ...
        {
            SSL_CTX_set_default_passwd_cb(mCTX, pemPasswdCallbackFunc);
            if (SSL_CTX_use_PrivateKey_file(mCTX, sPrivateKeyLocation.data(), SSL_FILETYPE_PEM) > 0) 
            {
                ...
                {
                    OsSysLog::add(FAC_KERNEL, PRI_ERR, "Private key '%s' does not match the certificate public key '%s'\n"
            ...
            {
                OsSysLog::add(FAC_KERNEL, PRI_ERR, "Private key '%s' could not be initialized. Is Password set?\n"
                              ,sPrivateKeyLocation.data()
                              );
                socketDescriptor = OS_INVALID_SOCKET_DESCRIPTOR;
            }
