...
OsStatus OsProcessMgr::setIORedirect(OsPath &rStdInputFilename, OsPath &rStdOutputFilename, OsPath &rStdErrorFilename)
{
...
OsStatus OsProcessMgr::startProcess(UtlString &rAlias, UtlString &rExeName, UtlString rParameters[],
                                    UtlString &startupDir, OsProcessBase::OsProcessPriorityClass prio,UtlBoolean bExeclusive)
{
    OsStatus retval = OS_FAILED;
    OsProcess process;
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"Attempting start on  %s\n",rAlias.data());
    ...
    {
        OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"Before addEntry for alias  %s\n",rAlias.data());
        retval = addEntry(rAlias,process.getPID());
        OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"Started OK for alias %s\n",rAlias.data());
    }
    else
        ...
        fprintf(stderr,"Error launching alias=%s\n",rAlias.data());
#endif
...
OsStatus OsProcessMgr::stopProcess(UtlString &rAlias)
{
    ...
    if (getProcessByAlias(rAlias,process) == OS_SUCCESS)
    {
        ...
        if (state == PROCESS_STARTED)
        {
            ...
            OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"Attempting stop on  %s\n",rAlias.data());
...
OsStatus OsProcessMgr::stopProcess(PID pid)
{
    ...
    {
        fprintf(stderr,"Error getting process in stopProcess(pid) pid=%d\n",pid);
    }
#endif
...
OsStatus OsProcessMgr::getProcessByAlias(UtlString &rAlias, OsProcess &rProcess)
{
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ENTERING getProcessByAlias  %s ",rAlias.data());

    lockAliasFile();

    if (loadProcessFile() == OS_SUCCESS)
    {
        OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"SUCCESS getProcessByAlias  %s loading process file",rAlias.data());
        UtlString value;
        if (pProcessList->get(rAlias,value) == OS_SUCCESS)
        {
            ...
            OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"getProcessByAlias  checking if pid %d for alias %s is valid",pid, rAlias.data());

            if (pid > 0)
            {
                retval = OsProcess::getByPID(pid,rProcess);
                if (retval == OS_SUCCESS)
                   OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"getProcessByAlias  alias %s pid %d IS VALID",rAlias.data(),pid);
                else
                   OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"getProcessByAlias  alias %s pid %d is NOT valid",rAlias.data(),pid);
        ...
        fprintf(stderr,"Error loading process files in getProcessByAlias alias=%s\n",rAlias.data());
#endif

    unlockAliasFile();

    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"EXITING getProcessByAlias  %s ",rAlias.data());
...
int OsProcessMgr::getAliasState(UtlString &rAlias)
{
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ENTERING getAliasState  %s ",rAlias.data());
    ...
    else
    {
        ...
        if (atoi(value) > 0)
        {
            ...
            OsProcess process;
            OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"getAliasState  checking if alias %s has valid PID",rAlias.data());
            if (getProcessByAlias(rAlias, process) == OS_FAILED)
            {
                OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ERROR getAliasState - alias %s DOES NOT HAVE a valid PID",rAlias.data());
                state = PROCESS_STARTING;
                setAliasState(rAlias,PROCESS_STARTING);
            }
            else
                 OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"getAliasState - alias %s HAS a valid PID",rAlias.data());

        }
        else
        {
            osPrintf("EXTREME ERROR: processAlias.dat file looks damaged!\n");
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"EXITING getAliasState %s",rAlias.data());
...
OsStatus OsProcessMgr::setAliasState(UtlString &rAlias,int newstate)
{
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ENTERING setAliasState  %s state = %d\n",rAlias.data(),newstate);

    switch(newstate)
    {
        case PROCESS_STARTING   : buf = "STARTING";
            break;
        case PROCESS_STOPPING   : buf = "STOPPING";
            break;
        case PROCESS_STOPPED    : buf = "STOPPED";
            break;
        case PROCESS_FAILED     : buf = "FAILED";
            break;
        case PROCESS_NEVERRUN   :
            ...
            break;
        default:
            osPrintf("ERROR: Invalid state passed to setAliasState!\n");
            break;
    }
    ...
    if (newstate != PROCESS_NEVERRUN)
    {
        ...
        {
            retval = OS_SUCCESS;
            OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"SUCCESS setAliasState  %s state = %d\n",rAlias.data(),newstate);
        }
        else
            OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ERROR setAliasState  %s state = %d\n",rAlias.data(),newstate);
    }

    unlockAliasFile();

    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"EXITING setAliasState  %s state = %d\n",rAlias.data(),newstate);
...
{               
    ...
    //now try to save it
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"Attempting addEntry");
    if (storeProcessFile() == OS_SUCCESS)
    {
        OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"addEntry to %s. PID=%d  Flushing processAlias.dat to disk. ",rAlias.data(),pid);
        retval = OS_SUCCESS;
    }
...
//    osPrintf("Locking: AliasLockCount=%d\n",AliasLockFileCount);
    ...
    while (!bIsHellFrozenOver)
    {
         ...
         OsSysLog::add(FAC_PROCESSMGR, PRI_DEBUG, "Trying to get lock on file: %s...", tmpStr.data());
        ...
        if (gFile_descr == -1)
        {
            ...
            if (i++ > 60)
            {
                ...
                {
                    OsSysLog::add(FAC_PROCESSMGR, PRI_ERR, "Remove successful.  Execution continuing...\n");
                }
                else
                {                 
                  OsSysLog::add(FAC_PROCESSMGR, PRI_ERR, "Error removing %s lock file.\n",lockFullPath.data());
                }
            }
        }
        else
...
OsStatus OsProcessMgr::storeProcessFile()
{
    ...
    OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"ATTEMPTING save on storeProcessFile %s",processFullPath.data());
    OsStatus savedCode = pProcessList->storeToFile(processFullPath.data());

    if (savedCode == OS_SUCCESS)
    {
         OsSysLog::add(FAC_PROCESSMGR, PRI_INFO,"SUCCESS saving in storeProcessFile %s",processFullPath.data());
         retval = OS_SUCCESS;
    }
    else
    {
         OsSysLog::add(FAC_PROCESSMGR, PRI_ERR,"ERROR %d saving in storeProcessFile %s ",
