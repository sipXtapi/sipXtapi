...
//QJOHUFM2 (PINGTEL1 shifted one letter up) 
const char pseed[] = {'Q','J','O','H','U','F','M','2'};
                                ...
                                UtlBoolean exceptOnErr)
{
   ...
   if (exceptOnErr)
   {
      switch (res)
      {
         ...
         case OS_NAME_IN_USE:
            throw new OsExcept(OsExcept::MAJOR_RUNTIME,
                               OsExcept::MINOR_RUNTIME,
                               "name already in use: " + rPrefix + rName,
                               "call to OsUtil::insertKeyValue");
            break;
         default:
            throw new OsExcept(OsExcept::MAJOR_RUNTIME,
                               OsExcept::MINOR_RUNTIME,
...
#ifdef _VXWORKS /* [ [ */
   switch (pingtelBoardType())
   {
#if CPU == ARMSA110 || (CPU == ARMARCH4) || (CPU == STRONGARM) /* [ [ */
   case PBT_BRUTUS: platform = PLATFORM_BRUTUS; break;
   case PBT_TCAS1:  platform = PLATFORM_TCAS1;  break;
   case PBT_TCAS2:  platform = PLATFORM_TCAS2;  break;
   case PBT_TCAS3:  platform = PLATFORM_TCAS3;  break;
   case PBT_TCAS4:  platform = PLATFORM_TCAS4;  break;
   case PBT_TCAS5:  platform = PLATFORM_TCAS5;  break;
   case PBT_TCAS6:  platform = PLATFORM_TCAS6;  break;
   default:
   case PBT_TCAS7:  platform = PLATFORM_TCAS7;  break;
#elif (CPU == XSCALE) /* ] [ */
   default:
   case PBT_TCAS8:  platform = PLATFORM_TCAS8;  break;
#else /* ] [ */
...
#ifdef _VXWORKS
   productType = PRODUCT_XPRESSA;
#elif defined(_WIN32) || defined(__linux__) || defined(sun)
...
// This will return a unique identfier for the platform this function is 
// executed from.
//
// For TCAS7 it will use a unique number burned into flash device.
// For TCAS6 - TCAS1 it will use the mac address
// For IX POSIX it will store the 8 bytes in /etc/keystore.key (root access needed)
// For IX Windows it will store the eight bytes in\Windows\keystore.key
//
// For all above cases the eight bytes are xor'd with a magic word before 
// giving them back to the user.
//
// Returns OS_FAILED if wrong platform or the key file could not be created or opened.

OsStatus OsUtil::getDeviceToken(UtlString &rTokenValue)
{
    UtlBoolean bGotSeed = FALSE;

    FILE *fptr;
    OsStatus retval = OS_FAILED;
    unsigned char seedbuf[8];
    UtlString keyStorePath = "/etc/keystore.key";  //default for linux
    
    memset(&seedbuf,0,sizeof(seedbuf));

#ifdef _VXWORKS
    //returns the current platform we are running on
    int dev = getPlatformType();

    if (dev == PLATFORM_TCAS7) //this device already has a unique number in it
    {
         //TODO: put eight unqiue bytes into seedbuf

    }

    //for all other TCASX platforms we will take the mac address
    UCHAR addr[6];

    int status = tcasMacAddr(addr);
    for (int loop = 0;loop < 8;loop++)
    {
        if (loop == 6)
            seedbuf[loop] ^= addr[0];  //reuse the first
            
        if (loop == 7)
            seedbuf[loop] ^= addr[5];  //reuse the last

        seedbuf[loop] ^= addr[loop];  
    }
    ...
    if (!bGotSeed)
    {
        ...
        char windirbuf[256];
        GetWindowsDirectory(windirbuf,sizeof(windirbuf));
        strcat(windirbuf,"\\keystore.key");
        keyStorePath = windirbuf;
#endif

        //for linux, we will try to load a unique value from a file named
        //keystore.key in the /etc dir.  If not there, we will try to create it
        // THIS WILL FAIL IF NOT EXECUTED AS ROOT
        fptr = fopen(keyStorePath,"rb");
        if (fptr)
        {
            //its there!  So read it in already...
            if (fread(seedbuf,1,8,fptr) == 8)
                retval = OS_SUCCESS;
            fclose(fptr);
        }
        else
        {
            //whoops it's not there, I guess we'll have to make one
        
            for (int loop = 0; loop < 8;loop++)
            {
                seedbuf[loop] += rand(); //get a random number;
            }
            
            fptr = fopen(keyStorePath,"wb+");
            if (fptr)
            {
                //write out the eight bytes
                if (fwrite(seedbuf,1,8,fptr) == 8)
                    retval = OS_SUCCESS;
                fclose(fptr);
            }
        }
    ...
    {
        //now xor this value with QJOHUFM2 (PINGTEL1 shifted one letter up) 
        for (int loop = 0; loop < 8;loop++)
        {
            seedbuf[loop] ^= pseed[loop];
        }

        char buf[32];
        sprintf(buf,"%02X%02X%02X%02X%02X%02X%02X%02X",
            seedbuf[0],seedbuf[1],seedbuf[2],seedbuf[3],
            seedbuf[4],seedbuf[5],seedbuf[6],seedbuf[7]);
