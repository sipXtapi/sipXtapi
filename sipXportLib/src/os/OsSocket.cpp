...
int OsSocket::read(char* buffer, int bufferLength)
{
   ...
   if (count > numForFailure)
   {
	...
	if(bytesRead < 0)
	{
		...
		if(error)
		{
			osPrintf("recv call failed with error: %d\n", error);
		}
	}
...
#define POLLSET(XX) ((pollState.revents & XX) != 0)
        ...
        while(1)
        {
            ...
            if (resCode == -1 && errno == EINTR &&
               // This socket may have been closed, so check
               // if the socket descriptor is still valid
               socketDescriptor > OS_INVALID_SOCKET_DESCRIPTOR)
               usleep(100);
            else
               break;
        ...
        if(resCode < 0)
        {
            ...
            OsSysLog::add(FAC_KERNEL, PRI_DEBUG, 
                "OsSocket::isReadyToRead poll returned %d (errno=%d) in socket: %d %p\n", 
        ...
        else if(resCode == 0)
        {
            ...
            {
                OsSysLog::add(FAC_KERNEL, PRI_DEBUG, 
                    "OsSocket::isReadyToRead poll returned %d in socket: %d 0%p\n", 
...
void OsSocket::getHostIpByName(const char* hostName, UtlString* hostAddress)
{
   ...
   else if (!hasDefaultDnsDomain() && (strchr(hostName, '.') == NULL))
   {
      *hostAddress = "0.0.0.0";
      osPrintf("getHostIpByName failed for %s %s\n   %s\n",
               hostName, hostAddress->data(),
               "hostName not fully qualified and no default domain");
   }
   else
   {
#if defined(_WIN32) || defined(__pingtel_on_posix__)
        ...
        if((dot2) && (dot2-dot1 > 1))
        {
            ...
            if((dot3) && (dot3-dot2 > 1))
            {
                ...
                if((dot4 == NULL) && strlen(dot3) > 1)
                {
                    ...
                    {
                        osPrintf("inet_addr: %d\n", (int)inet_addr(address));
                    }
                }
            }
        }
