  ...
  // construct thread attribute
   ...
   if(sEffectiveUid != 0 && linuxPriority != RT_NO)
   {
      OsSysLog::add(FAC_KERNEL, PRI_WARNING, 
                    "OsTaskLinux:doLinuxCreateTask unable to set RT linux priority: %d of task: %s", 
                    linuxPriority, mName.data());
   }
   if(sEffectiveUid == 0 && linuxPriority != RT_NO)
   {
...
void * OsTaskLinux::taskEntry(void* arg)
{
   ...
   if (sEffectiveUid == 0 && sRealUid != 0)
   {
      ...
      if (0 == geteuid())
      {
         OsSysLog::add(FAC_KERNEL, PRI_WARNING,  "unable to set UID for task %s",
