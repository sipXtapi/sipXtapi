...
UtlBoolean OsTaskLinux::sUidSet = FALSE;
uid_t OsTaskLinux::sEffectiveUid = 10000;
uid_t OsTaskLinux::sRealUid = 10000;
...
{
   // These seem to cause some problems... I don't know why. -MM
   //requestShutdown();
   //waitUntilShutDown();
...
/* This code would enable realtime priority threads, which is silly. */
...
UtlBoolean OsTaskLinux::doLinuxCreateTask(const char* pTaskName)
{
   ...
   // Only if the effective user id is root (0) can we use realtime
   // scheduling and priorities

   if(!sUidSet)
   {
      sEffectiveUid = geteuid();
      sRealUid = getuid();
      sUidSet = TRUE;
   }
  ...
  // construct thread attribute
   ...
   int linuxPriority = OsUtilLinux::cvtOsPrioToLinuxPrio(mPriority);
   // Can only set the priorities on linux if you are root
   if(sEffectiveUid == 0 && linuxPriority != RT_NO)
   {
      // Use round robin realtime scheduling
      pthread_attr_setschedpolicy(&attributes, SCHED_RR); 
      param.sched_priority = linuxPriority;
      OsSysLog::add(FAC_KERNEL, PRI_INFO, 
                    "OsTaskLinux:doLinuxCreateTask starting %s at linux priority: %d", 
                    mName.data(), linuxPriority);
   ...
   {
      OsSysLog::add(FAC_KERNEL, PRI_ERR, "pthread_create failed, returned %d in %s (%p) \n", linuxRes, mName.data(), this);
      return FALSE;
   }
}
...
int setVTSusp(int susp) {return setLinuxTaskStartSuspended(susp);}
...
void * OsTaskLinux::taskEntry(void* arg)
{
   ...
   if (linuxTaskStartSuspended) pthread_kill(pthread_self(), SIGSTOP);

   // If the task has the suid bit set and we are running as root
   // (to be able to use realtime scheduling) set the uid back to
   // avoid security issues
   if(sEffectiveUid == 0 && sRealUid != 0)
   {
      seteuid(sRealUid);
   }
   if(0 == geteuid())
   {
      OsSysLog::add(FAC_KERNEL, PRI_WARNING,  "task %s running as root",
                    pTask ? pTask->mName.data() : "(null)");
   }
...
void OsTaskLinux::taskUnregister(void)
{
   ...
   if (res != OS_SUCCESS)
   {
      osPrintf("\nWARNING! OsTaskLinux::doLinuxTerminateTask, failed to delete"
