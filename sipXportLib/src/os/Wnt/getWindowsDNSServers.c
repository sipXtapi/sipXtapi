...
// MACROS
// EXTERNAL FUNCTIONS
extern void osPrintf(char* format, ...);
...
static HMODULE loadIPHelperAPI()
{
   ...
   if (hRetModule == NULL)
   {
      ...
      if (!hRetModule)
         osPrintf("Cannot find iphlpapi.dll!\n");
    }
      ...
      if (hModule && GetNetworkParams)
      {
            ...
            if( retErr == ERROR_BUFFER_OVERFLOW )
         {
             ...
             if( ( pNetworkInfo = (PFIXED_INFO)GlobalAlloc( GPTR, dwNetworkInfoSize ) ) != NULL )
            {
               ...
               {
                   osPrintf( "DNS ERROR: GetNetworkParams failed with error %d\n", retErr );
                   GlobalFree(pNetworkInfo);   // handle to global memory object
                }
            }
            else
            {
                osPrintf( "DNS ERROR: Memory allocation error\n" );
             }
         }
         else
                osPrintf( "DNS ERROR: GetNetworkParams sizing failed with error %d\n", retErr );
...
static int getDNSEntriesFromRegistry(char regDNSServers[][MAXIPLEN], int max)
{
   ...
   {
      osPrintf("Error reading values from registry in func: getDNSEntriesFromRegistry\n"); 
   }
