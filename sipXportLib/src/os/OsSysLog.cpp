                        ...
                        const va_list          ap)
{
   ...
   if (spOsSysLogTask != NULL)
   {
      if (willLog(facility, priority))
      {
         ...
         OsDateTime time ;
         UtlString   strTime ;
         OsDateTime::getCurTime(time) ;
         time.getHttpTimeString(strTime) ;
//#ifdef LOG_USEC
         int uSec = time.usecs();
         char uSecChars[20];
         sprintf(uSecChars, ".%06d", uSec);
         strTime.append(uSecChars);
//#endif
  ...
  setOutputFile(0, logname) ;  
   ...
   for (int i=0; i<iEntries; i++)
   {
      if (logLevel == lkupTable[i].pIdentity)
      {
         ...
         add(facility, PRI_INFO, "Setting %s syslog level : %s", sFacilityNames[facility], lkupTable[i].pIdentity) ;
         break;
      }
   }
