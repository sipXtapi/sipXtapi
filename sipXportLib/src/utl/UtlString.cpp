...
// Constructor accepting an initial capacity. 
UtlString::UtlString(int initCapacity)      
{
    mpData = NULL;
    mSize = 0;
    mCapacity = 0;
...
{
    //if N>strlen(szStr), still append N bytes.
    if(szStr && N>0) 
    {   
        // Make sure there is room for one extra null
...
UtlString& UtlString::append(const UtlString& str)  
{
    ...
    // Do not assume the first '\0' is the end of the
    // string.  There may be binary data in the string
    return append(str.mpData, str.mSize);
}
...
{   
    ...
    if(mCapacity < N && N > 0)   
    {
        if(mCapacity + mIncrement > N)
        {
            N = mCapacity + mIncrement;
        }
#ifdef _VXWORKS
...
UtlString UtlString::operator() (size_t start, size_t len) const   
{
    ...
    if (mpData && start + len < mSize)
    {
        newUtlString.append(&mpData[start], len);
    }
    else if(mpData && start < mSize )  //get all of after start
    {
