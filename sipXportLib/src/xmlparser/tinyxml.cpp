...
bool TiXmlDocument::LoadFile( const char* filename )
{
   ...
   /*
    Comment by Pradeep:
    Opening the file and reading the bytes was done very inefficiently
    here. I changed it to use OsFile to read the file in one chunk and
    parse those bytes rather than bothering to append chunk of bytes to
    TIXML_STRING. Appending to TIXML_STRING is not efficient.
    Perforce Changelist 34506.
    */

    if ( OsFileSystem::exists( filename ) )
    {
        OsStatus rc;
        OsFile importFile ( filename );
        if ((rc = importFile.open( OsFile::READ_ONLY )) == OS_SUCCESS )
        {
            ...
            if ( (rc = importFile.getLength( length )) == OS_SUCCESS )
            {
               if (length > 0)
               {
                  ...
                  if (pBuf && (rc = importFile.read(pBuf, length, bytesRead)) == OS_SUCCESS )
                  {   // sets the document error value if parsing encounters a problem
                     pBuf[length] = 0;
                     if (Parse( pBuf )) ret = true;
                  ...
                  {
#                     ifdef TEST_PRINT
                      fprintf(stderr, "tinyxml: read error %d: '%s'\n",rc, filename);
#                     endif
                  }
               }
               else
               {
#                  ifdef TEST_PRINT
                   fprintf(stderr, "tinyxml: zero length file '%s'\n", filename);
#                  endif
               }
            }
            else
            {
#               ifdef TEST_PRINT
                fprintf(stderr, "tinyxml: length error %d: '%s'\n",rc, filename);
#               endif
            }
        }
        else
        {
#          ifdef TEST_PRINT
           fprintf(stderr, "tinyxml: cannot open: '%s'\n",filename);
#          endif
        }
    }
    else
    {
#       ifdef TEST_PRINT
        fprintf(stderr, "tinyxml: file does not exist: '%s'\n",filename);
#       endif
    }


    if (!ret) SetError( TIXML_ERROR_OPENING_FILE );
