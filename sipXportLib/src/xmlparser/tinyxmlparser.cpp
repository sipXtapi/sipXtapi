...
/*
* changed made to parse more efficiently.
* When parsing a value of a text node like
* <identity>jsmith@acme.com</identity>,
*  TiXml was reading a char and appending it to a
*  TIXML_STRING for each char of the text value "jsmith@ame.com".
*  I made it to read all the characters first and
*  then append it all at once to a TIXML_STRING.
    ...
    *text = "";
                ...
                if( p && *p )
      {
         while (           p && *p
            ...
            {
               const char* iOriginalPosition = p;
               int iLength = 0;
               ...
               (* text).append( iOriginalPosition, iLength );
            }
        ...
        else
        {
            ...
            while( !(*p == '\r' || *p == '\n') && !isspace(*p) && !StringEqual( p, endTag, caseInsensitive ))
            {
                  if( *p == '&' )
                  {
                     char c;
...
const char* TiXmlDocument::Parse( const char* p )
{
        ...
        // @JC This is bad code, change to return a
        // null pointer
                return /* false */NULL;
        }
