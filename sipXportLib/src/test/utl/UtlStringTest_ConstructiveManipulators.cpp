...
//// ========================= BEGIN TEST CASES ==============================
    ...
    */
    ...
    void testConstructor_2()
    {
        ...
        for (int i = 0 ; i < testCount ; i++)
        {
            ...
            UtlString testString(testData[i].inputCapacity) ;
            
            // Verify that the string's data is empty
            TestUtilities::createMessage(3, &Message, prefix, \
                           testData[i].testDescription, suffix1) ; 
            CPPUNIT_ASSERT_EQUAL_MESSAGE(Message.data(), string(""), \
                           string(testString.data())) ;

            // Verify that the string's capacity has been set to the value passed in the constructor
            TestUtilities::createMessage(3, &Message, prefix, \
                           testData[i].testDescription, suffix2) ; 
            CPPUNIT_ASSERT_MESSAGE(Message.data(), \
                               testString.capacity() >=  testData[i].minExpectedCapacity) ;
        }
    } //testConstructor2()
    ...
    */
    ...
    {
        typedef struct InsertTestData 
        {
            const char* szSource ;
            const char* szInsert ;
            const int   iPosition ;
            const char* szResults ;
        } ;

        #define INSERT_TEST_DATA_LENGTH     7
        InsertTestData data[INSERT_TEST_DATA_LENGTH] =
        {
