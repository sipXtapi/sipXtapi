...
public:
    ...
    // To be returned from a search that failed.
    static const size_t UTLSTRING_NOT_FOUND;
    // When specifying the length of a substring, indicates that the substring
    // should extend to the end of the string.
...
///@{
    ...
    UtlString(int capacity  /**< minimal preallocated size for this string */ );
    /**<
     * The initial capacity is used to preallocate storage allowing for multiple
     * appends without reallocating memory.
     */
...
///@{
    ...
    /// Allows references of the form stringVar(start,length)
