...
[
   AC_ARG_VAR(CLOVER_HOME, [Clover home directory])

   if test x_"${CLOVER_HOME}" != x_
   then
       clover_jar=${CLOVER_HOME}/lib/clover.jar
       AC_CHECK_FILE([$clover_jar],
       [
           found_clover="yes"
       ],)
       if test x_$found_clover != x_yes; then
               AC_MSG_ERROR([Invalid CLOVER_HOME environment variable: Cannot find $clover_jar.])
       fi
   fi
])
