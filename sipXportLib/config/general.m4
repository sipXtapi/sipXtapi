...
[
   ...
   for dir in $JBOSS_HOME $OPT_JBOSS_HOME $LOCAL_JBOSS_HOME /usr/local/jboss; do
       j2ee_jar=$dir/jboss/client/jboss-j2ee.jar
   ...
   if test x_$found_jboss != x_yes; then
       AC_MSG_ERROR([JBoss not installed. Set JBOSS_HOME env. 
           ...
           something similiar to JBoss-2.4.10_Tomcat-4.0.6])
   fi
])
...
[   AC_ARG_WITH(openssl,
    ...
    if test x_$found_ssl_inc != x_yes ; then
        ...
        CXXFLAGS="$CXXFLAGS -DHAVE_SSL"
        # don't need to add -I/usr/include 
        if test "${sslincdir}" != "/usr/include"; then
           CFLAGS="$CFLAGS -I$sslincdir"
           CXXFLAGS="$CXXFLAGS -I$sslincdir"
        fi
    fi
    ...
    if test x_$found_ssl_lib != x_yes ; then
        ...
        LDFLAGS="$LDFLAGS -L$ssllibdir -lssl -lcrypto"
    fi
...
## openssl-devel rpm installs kerbose in another dir
    ...
    if test x_$krb_found = x_yes; then
        ...
        CFLAGS="$CFLAGS -I$krbdir/kerberos/include"
        CXXFLAGS="$CXXFLAGS -I$krbdir/kerberos/include"
    else
        AC_MSG_RESULT(['krb5.h' not found])
    fi
])
...
[   AC_MSG_CHECKING([for xerces])
    ...
    for dir in $withval /usr/local/xercesc /usr/lib/xercesc /usr/xercesc /usr/pkg /usr/local /usr; do
        xercesdir="$dir"
...
[   
   ...
   AC_ARG_WITH([apache-modules],
               ...
               [ apache2_mod_search_path="/usr/local/apache2/modules /usr/apache2/modules /etc/httpd/modules /usr/lib/httpd/modules /usr/lib/apache2/modules"
   ...
   AC_ARG_WITH([apache-home],
               ...
               [ apache2_home_search_path="/usr/local/apache2 /usr/apache2 /etc/httpd /usr/local/sbin /usr/local /usr/sbin /usr /usr/lib/apache2"
                 ]
               )        
...
[
    ...
    for cgicc_dir in $cgicc_path; do
	   if test -f "$cgicc_dir/lib/libcgicc.so"
       then 
         found_cgicc="yes";
         break;
      fi
    ...
    then
        ...
        AC_SUBST(CGICC_DIR,$cgicc_dir)
    else
        AC_MSG_ERROR([cgicc not found; tried $cgicc_path])
    fi
])dnl
...
[   AC_MSG_CHECKING([for libwww])
    ...
    if test x_$found_www != x_yes; then
        AC_MSG_ERROR(Cannot find libwww includes)
    fi
    if test ! -e "$dir/lib/libwwwapp.so";then
        AC_MSG_ERROR(Cannot find libwww libraries in $dir/lib)
    fi
    if test ! -e "$dir/lib/libwwwssl.so";then
        AC_MSG_ERROR(Cannot find libwww ssl libraries in $dir/lib)
    fi
    if test x_$found_www = x_yes; then
