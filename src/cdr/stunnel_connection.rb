  ...
  def open(config)
    ...
    if @ha_enabled
      if ! check_stunnel_running
        ...
        Utils.raise_exception("An instance of stunnel is already running with Pid #{@pid}. It must be shut " +
  ...
  def get_stunnel_config(config)
    ...
    if @ha_enabled
      ...
      if ! test(?e, "#{@prefix}/#{SIPXPBX_SSLPATH}/authorities/#{ca_file}")
        Utils.raise_exception("CA file \"#{@prefix}/#{SIPXPBX_SSLPATH}/authorities/#{ca_file}\" does not exist.")
      end
