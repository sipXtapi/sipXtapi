...
SubscribeServerThread::handleMessage(OsMsg& eventMessage)
{
    ...
    if (!message->isResponse())
    {
        ...
        {
            UtlString eventType;
            message->getEventField( eventType );
               ...
               if( isAuthenticated ( message, &finalResponse, authenticatedUser, authenticatedRealm ) )
               {
                  if ( isAuthorized ( message, &finalResponse, pluginContainer ) )
                  {
                     ...
                     if (plugin)
                     {
                        ...
                        int isSubscriptionAdded = addSubscription( timeNow,
                                                                   message,
                                                                   mDefaultDomain,
                                                                   finalResponse);
        ...
        else // Invalid domain
        {
            ...
            mpSipUserAgent->getContactUri(&localContactUriString);
            ...
            {
                syslog(FAC_SIP, PRI_WARNING, "Loop Detected while forwarding the request\n");
                finalResponse.setResponseData(
                    message,
            ...
            else
            {
                ...
                {
                    syslog(FAC_SIP, PRI_WARNING, "Forwarded invalid domain request to correct domain\n");

                    // Forward the register onward to the destination proxy
                    // When the SIP SubscribeServerThread receives a REGISTER request
                    // for a domain not supported by the SubscribeServerThread, it should
                    // remove the loose route that the forking proxy added.
                    ...
                    mpSipUserAgent->send(forwardedMessage);
                }
            }
        }
    } 
    else // response
    {
       ...
       if ( SIP_BAD_TRANSACTION_CODE == message->getResponseStatusCode() )
       {
          // remember if this comes back we must delete it
          ...
          if ( pRequest != NULL )
          {
             ...
             if( pluginContainer != NULL )
             {
                ...
                syslog(FAC_SIP, PRI_DEBUG, "SubscribeServerThread::handleMessage() -"
                       " Processing 481 response from phone\n");
    ...
    SipMessage* responseMessage )
{
    ...
    // @JC Fix to compare against either an IP address of the
...
int SubscribeServerThread::addSubscription(
    const int timeNow,
    ...
    SipMessage& response)
{
    ...
    subscribeMessage->getEventField(eventType);
       ...
       if( commonExpires > 0 ) // came from request
       {
          if (commonExpires < mMinExpiresTimeint) 
          {
             ...
             OsSysLog::add( FAC_SIP, PRI_DEBUG, "SubscribeServerThread::addSubscription -"
                            " Expires Value less than Min Expires");
             return returnStatus;
          }
    ...
    UtlString subfileBase( callId );
    subfileBase.append( to );
    subfileBase.append( from );

    NetMd5Codec::encode( subfileBase.data(), subscribeFileName );
    subscribeFileName.append(".txt");
    //@TODO - store file
    ...
    if ( !SubscriptionDB::getInstance()->
            insertRow( requestUri, // identity,
                       ...
                       to,
                       from,
                       subscribeFileName,   // plugin specific filtering opts.
...
int SubscribeServerThread::removeSubscription ( const SipMessage& sipMessage ) const
{
    int returnStatus = STATUS_SUCCESS;
    int subscribeCseqInt = 0;
    UtlString callId;
