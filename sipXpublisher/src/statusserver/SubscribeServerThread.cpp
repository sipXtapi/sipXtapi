    ...
    SipMessage* responseMessage )
{
    ...
    {
        OsSysLog::add(FAC_AUTH, PRI_DEBUG, "SubscribeServerThread::isValidDomain() - VALID Domain '%s'",
            address.data());
        isValid = TRUE;
    }
    ...
    if (!isValid)
    {
       ...
       {
          OsSysLog::add(FAC_AUTH, PRI_DEBUG, "SubscribeServerThread::isValidDomain() VALID virtual domain '%s'",
              address.data());
          isValid = TRUE;
       }
    }
    ...
    if (!isValid)
    {
       OsSysLog::add(FAC_AUTH, PRI_DEBUG, "SubscribeServerThread::isValidDomain() - INVALID Domain '%s'",
           address.data());
    }
