...
/* ============================ MANIPULATORS ============================== */
     ...
     * set. Currently three types of media players are supported: 
     * \par
     * 1) A simple player that buffers some of the media and then 
     * starts playing a single media source (URL or stream).
     * \par
     * 2) A list player that buffers media sources as they are 
     * added and plays them as soon as all previously added 
     * sources are complete.
     * \par
     * 3) A queued player that supports two buffered play lists 
    ...
    virtual void setOutboundLineForCall(const char* callId, const char* address) = 0;
    ...
    //! Get the SIP session or dialog information for the specified terminal connection.
