...
CallManager::~CallManager()
{
    ...
    if(speedNumberList)
    {
        delete speedNumberList;
        speedNumberList = NULL;
    }
...
UtlBoolean CallManager::handleMessage(OsMsg& eventMessage)
{
   ...
   CpPhoneMediaInterface* pMediaInterface;
        ...
        switch(msgSubType)
        {
         ...
         case CP_SIP_MESSAGE:
         {
            OsWriteLock lock(mCallListMutex);                
               ...
               UtlString callId;
#ifdef ENABLE_MGCP /* [ */
               if(msgSubType == CP_MGCP_MESSAGE)
               {
                  int numCodecs;
                  SdpCodec** codecArray = NULL;
                  getCodecs(numCodecs, codecArray);

                  // Create a new MGCP call                       
#error This code has been changed.  mPublicAddress is now contained in SipUserAgent. Youll need to update this code to use it.

                  pMediaInterface = new CpPhoneMediaInterface(mRtpPortStart, mRtpPortEnd,
                        mPublicAddress.data(), mLocalAddress.data(),
                        numCodecs, codecArray, mLocale.data(), mExpeditedIpTos);
                  handlingCall = new CpMgcpCall(this, pMediaInterface, 
                        aquireCallIndex(), mpMgcpStackTask);
               ...
               if(msgSubType == CP_SIP_MESSAGE)
               {
                  ...
                  {
                     // we do not want to exceed the number of calls to exceed
                     // the number of flowgraphs that can be created.
                     // if that is the case, send a BUSY_HERE sip message back 
                     // to sender.
                     if(getCallStackSize() >= MpMediaTask::maxNumManagedFlowGraphs())
                     {
                     ...
                     {                        
                        ...
                        pMediaInterface = new CpPhoneMediaInterface(mRtpPortStart, mRtpPortEnd,
                              publicAddress.data(), mLocalAddress.data(),
                              numCodecs, codecArray, mLocale.data(), mExpeditedIpTos);
            ...
            case CP_CALL_EXITED:
         {
            ...
            ((CpIntMessage&)eventMessage).getIntData((int&) call);
                OsSysLog::add(FAC_CP, PRI_DEBUG, "Call EXITING message received: %p\r\n", (void*)call);
                OsSysLog::add(FAC_CP, PRI_DEBUG, "infocusCall: %p\r\n", infocusCall);
                
            if(infocusCall == call)
            {
               call->stopMetaEvent();
                    releaseCallIndex(infocusCall->getCallIndex());
                    OsSysLog::add(FAC_CP, PRI_DEBUG, "Deleting infocusCall\r\n");
                ...
                else
            {
               call = removeCall(call);
                  ...
                  OsSysLog::add(FAC_CP, PRI_DEBUG, "Shutting down call\n");
                  call->requestShutdown();
                        OsSysLog::add(FAC_CP, PRI_DEBUG, "Deleting call from call stack\r\n");
                        delete call;
                        call = NULL;
                  OsSysLog::add(FAC_CP, PRI_DEBUG, "Call shutdown and deleted\n");
               }
            }
            messageProcessed = TRUE;
            break;
         ...
         case CP_DIAL_STRING:
         {
            ...
            if(infocusCall && dialing)
            {
               ...
               ((CpStringMessage&)eventMessage).getStringData(mDialString);
         ...
         case CP_CONNECT:
         {
            ...
            doConnect(callId.data(), addressUrl.data());
            messageProcessed = TRUE;
            break;
         }
         ...
         // Forward the message to the call
         {
            ...
            if(!call)
            {
                     ...
                     msgSubType == CP_PLAY_BUFFER_TERM_CONNECTION)
               {
                  ...
                  if (eventWithoutCall)
                  {
                     OsSysLog::add(FAC_CP, PRI_ERR, "Signaling event for message subtype %d request on invalid callId '%s'\n",
                  ...
                  {
                     OsSysLog::add(FAC_CP, PRI_ERR, "No event to signal for message subtype %d request on invalid callId '%s'\n",
                           msgSubType, callId.data());
                  }
               }
...
{
    if(callId->isNull()) getNewCallId(callId);
...
PtStatus CallManager::transfer(const char* callId, const char* transferToUrl,
                               ...
                               MpStreamPlayer** ppPlayer)
{
   ...
   switch (type)
   {
      case MpPlayer::STREAM_PLAYLIST_PLAYER:
         msgtype = CP_CREATE_PLAYLIST_PLAYER;
         break;
...
void CallManager::destroyPlayer(int type, const char* callId, MpStreamPlayer* pPlayer)
{
   ...
   {
      case MpPlayer::STREAM_PLAYLIST_PLAYER:
         msgtype = CP_DESTROY_PLAYLIST_PLAYER;
         break;
   ...
   {
      case MpPlayer::STREAM_PLAYLIST_PLAYER:         
         delete (MpStreamPlaylistPlayer*) pPlayer ;
         break;
      case MpPlayer::STREAM_QUEUE_PLAYER:        
...
void CallManager::setOutboundLineForCall(const char* callId, const char* address)
{
    CpMultiStringMessage acceptMessage(CP_SET_OUTBOUND_LINE, callId, address);
    postMessage(acceptMessage);
}

void CallManager::acceptConnection(const char* callId, const char* address)
{
...
void CallManager::dialString(const char* url)
{
    if(url && strlen(url) > 0)
    {
        ...
        CpStringMessage dialEvent(CP_DIAL_STRING, trimmedUrl.data());
                                ...
                                const char* cause)
{
    ...
    if(mCallStateLogEnabled)
    {
        ...
        if ((mCallStateLog.length() + len + 100) > MAXIMUM_CALLSTATE_LOG_SIZE)
        {
            mCallStateLog.remove(0);
#ifdef TEST_PRINT
        OsSysLog::add(FAC_CP, PRI_DEBUG, "Call State message CLEARED because it reached max size (%d)", MAXIMUM_CALLSTATE_LOG_SIZE);
#endif
        }
        ...
        time( &ltime );
        sprintf(buf, "\nTime: %s", ctime(&ltime));
        mCallStateLog.append(buf);

        mCallStateLog.append("\n--------------------END--------------------\n");
    }
#ifdef TEST_PRINT
    else OsSysLog::add(FAC_CP, PRI_DEBUG, "Call State LOGGING DISABLED\n");
#endif
}
                             ...
                             UtlBoolean assumeFocusIfNoInfocusCall)
{
    ...
    else
    {
        if(mOutGoingCallType == SIP_CALL)
        {
            ...
            CpPhoneMediaInterface* mediaInterface = 
                new CpPhoneMediaInterface(mRtpPortStart, mRtpPortEnd,
                                          publicAddress.data(), mLocalAddress.data(),
                                          numCodecs, codecArray, mLocale.data(), mExpeditedIpTos);
...
void CallManager::doConnect(const char* callId, const char* addressUrl)
{
    CpCall* call = findHandlingCall(callId);
    if(!call)
    {
    ...
    else
    {
        ...
        CpStringMessage dialStringMessage(CP_DIAL_STRING, addressUrl);
