...
#error This code has been changed.  mPublicAddress is now contained in SipUserAgent. Youll need to update this code to use it.
         ...
         case CP_CONNECT:
         {
            ...
            doConnect(callId.data(), addressUrl.data(), desiredConnectionCallId.data());
            messageProcessed = TRUE;
            break;
         }
                              ...
                              const char* desiredCallIdString)
{
    ...
    if(returnCode == PT_SUCCESS)
    {
        CpMultiStringMessage callMessage(CP_CONNECT, callId, 
            toAddressUrl, fromAddressUrl, desiredCallId);
        postMessage(callMessage);
    }
    return(returnCode);
}
...
void CallManager::doConnect(const char* callId, const char* addressUrl, const char* desiredConnectionCallId)
{
    CpCall* call = findHandlingCall(callId);
    if(!call)
    {
    ...
    else
    {
        ...
        CpMultiStringMessage dialStringMessage(CP_DIAL_STRING, addressUrl, desiredConnectionCallId);
