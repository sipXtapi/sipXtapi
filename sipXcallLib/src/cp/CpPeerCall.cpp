...
CpPeerCall::~CpPeerCall()
{
    ...
    if (!mCallId.isNull())
    {
       ...
       osPrintf("CpPeerCall-%s destructor: %s\n", name.data(), mCallId.data());
    }
    else
    {
       OsSysLog::add(FAC_CP, PRI_DEBUG, "CpPeerCall-%s destructor:: callId is Null\n", name.data());
       osPrintf("CpPeerCall-%s destructor:: callId is Null\n", name.data());
    }
#endif
    ...
    {   
       ...
       osPrintf("Leaving CpPeerCall-%s destructor: %s\n", name.data(), mCallId.data());
    }
    else
    {
       OsSysLog::add(FAC_CP, PRI_DEBUG, "Leaving CpPeerCall-%s destructor:: callId is Null\n", name.data());
       osPrintf("Leaving CpPeerCall-%s destructor:: callId is Null\n", name.data());
    }
#endif
}
...
UtlBoolean CpPeerCall::handleDialString(OsMsg* pEventMessage)
{
    ...
    // Always stop the tone incase something else was playing
    // i.e. dialtone or a previous key
    ((CpStringMessage*)pEventMessage)->getStringData(dialString);
#ifdef TEST_PRINT
    ...
    addHistoryEvent("CP_DIAL_STRING (3) \n\tDialString: \"" + dialString + "\"");
    {
     ...
     //                     "Dialing complete");
        //osPrintf("INVITING\n");
        // Need to do the real SIP INVITE stuff here
        // The tone will not be generated here, it will ringback when
        // the INVITE response ringing is received
        // Do not ring until the SIP message comes back saying that
        // it is ringing
        ...
        { 
            addParty(remoteHostName.data(), NULL, NULL);
        } 
    }
    return TRUE ;
}
...
#endif
            addParty(referTo, referredBy, originalConnectionAddress);
                               ...
                               const char* originalCallConnectionAddress)
{
    ...
    getCallId(callId);
    // Use a different call-id for each connection
    //mpManager->getNewSessionId(&callId);

    connection->dial(transferTargetAddress,
                    mLocalAddress.data(), callId.data(),
                    callController,
