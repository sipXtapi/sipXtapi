            ...
            case CallManager::CP_CREATE_QUEUE_PLAYER:
               {
                  ...
                  OsSysLog::add(FAC_CP, PRI_DEBUG, 
                        "CpCall::handle creating players ppPlayer 0x%08x ev 0x%08x flags %d", 
                  ...
                  if (msgSubType == CallManager::CP_CREATE_PLAYER)
                  {                     
                     mpMediaInterface->createPlayer((MpStreamPlayer**)ppPlayer, streamId, flags, mpManager->getMessageQueue(), callId.data()) ;
                  }
                  else if (msgSubType == CallManager::CP_CREATE_QUEUE_PLAYER)
                  {
                     mpMediaInterface->createQueuePlayer((MpStreamQueuePlayer**)ppPlayer, mpManager->getMessageQueue(), callId.data()) ;
                  }
                  else if (msgSubType == CallManager::CP_CREATE_PLAYLIST_PLAYER)
                  {
                     mpMediaInterface->createPlaylistPlayer((MpStreamPlaylistPlayer**)ppPlayer, mpManager->getMessageQueue(), callId.data()) ;
                  }
            ...
            case CallManager::CP_DESTROY_QUEUE_PLAYER:
               {
                  ...
                  if(mpMediaInterface)
                  {
                     MpStreamPlayer* pPlayer = (MpStreamPlayer*) ((CpMultiStringMessage&)eventMessage).getInt2Data();
                     if (msgSubType == CallManager::CP_DESTROY_QUEUE_PLAYER)
                        mpMediaInterface->destroyQueuePlayer((MpStreamQueuePlayer*)pPlayer) ;
                     else if (msgSubType == CallManager::CP_DESTROY_PLAYLIST_PLAYER)
                        mpMediaInterface->destroyPlaylistPlayer((MpStreamPlaylistPlayer*)pPlayer) ;
                     else // CP_DESTROY_PLAYER
