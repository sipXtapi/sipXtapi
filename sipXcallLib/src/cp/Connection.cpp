  ...
  UtlString strCallName;
   ...
   if ( strCallName.isNull())
	...
	if (!isStateTransitionAllowed(newState, currentState))
	{
		...
		{
			osPrintf("Connection state change - isLocal %d\n\t for call %s\n\t with callid %s\n\t from: %s\n\t to %s\n\t (cause=%d) is not allowed.\n",
				isLocal, strCallName.data(), callId.data(), oldStateString.data(), newStateString.data(), newCause);
			return;
		}
	}
...
#ifdef TEST_PRINT 
   osPrintf("Call %s %s state isLocal %d \n\t change \n\tfrom %s to\n\t %s \n\t(cause=%d)\n\t changeState %d\n\t post change to upper layer %d\n",
           strCallName.data(), 
           callId.data(), 
           isLocal,
           oldStateString.data(), 
	...
	if (changeState)
	{
		...
		if (termState == -1)
			mTerminalConnState = terminalConnectionState(newState);
		else
			mTerminalConnState = termState;
   ...
   if (bPostStateChange)
	...
	oldStateString = OsUtil::NULL_OS_STRING;
	newStateString = OsUtil::NULL_OS_STRING;
}
...
{
//	osPrintf("**** get 0x%08x state %d\n", (int)this, mConnectionState);
	int state;
       ...
       state != mLocalConnectionState)
   {
       ...
       OsSysLog::add(FAC_CP, PRI_WARNING, "Connection::getState: local connection(%s) may be terminated (remote %s) ", oldStateString.data(), newStateString.data());
       osPrintf("Connection::getState: local connection(%s) may be terminated (remote %s) \n", oldStateString.data(), newStateString.data());
       state = mLocalConnectionState;
   } 
             ...
             mRemoteConnectionState != state)
   {
       ...
       getStateString(state, &newStateString);
       OsSysLog::add(FAC_CP, PRI_WARNING, "Connection::getState: remote connection(%s) may be terminated (local %s) ", oldStateString.data(), newStateString.data());
       osPrintf("Connection::getState: remote connection(%s) may be terminated (local %s) \n", oldStateString.data(), newStateString.data());
       state = mRemoteConnectionState;
   }
...
void Connection::getCallId(UtlString* callId)
{
	OsLock lock(callIdMutex);
	callId->remove(0);
	callId->append(connectionCallId.data());
}

void Connection::setCallId(const char* callId)
{
	OsLock lock(callIdMutex);
	connectionCallId.remove(0);
	connectionCallId.append(callId);
}
...
void Connection::getCallerId(UtlString* callerId)
{
   OsLock lock(callIdMutex);
	callerId->remove(0);
	callerId->append(connectionCallerId.data());
}

void Connection::setCallerId(const char* callerId)
{
	OsLock lock(callIdMutex);
	connectionCallerId.remove(0);
	connectionCallerId.append(callerId);
}
