...
die "Arguments must be commands or control file names.\n"
    if $#ARGV < 0;

# Process the arguments, which are either file names or commands.
$errors_found = 0;
for ($i = 0; $i <= $#ARGV; $i++) {
    $arg = $ARGV[$i];
...
print "Ending run after ", time - $start_time, " seconds.\n";
...
print "Done after ", time - $start_time, " seconds.\n";
...
sub start_job {
  ...
  find_script: for ($tries = 0; $tries < $script_select_tries; $tries++) {
	  ...
	  if ($r <= $script_cuts[$script_no]) {
	      ...
	      # Check if we should not run duplicates, and if so, check if this
	      # is a duplicate of a script now running.
	      if ($no_duplicates) {
		  for ($i = 0; $i < $job_limit; $i++) {
	...
	exec(@command);
	die "Error from exec($CallScript_program ...): $!\n";
    }
...
sub process_command {
    ...
    if ($tokens[0] eq 'time') {
	# time NNN
	$time_limit = $tokens[1] + 0;
	unless ($time_limit > 0) {
	    print STDERR "Invalid time value '$tokens[1]' in $id\n";
	    $errors_found++;
	    return;
	}
	...
	unless ($CallScript_program ne '' && -x $CallScript_program) {
	    print STDERR "Invalid program name '$CallScript_program' in $id\n";
	    $errors_found++;
	    return;
	}
	print "\$job_limit = $job_limit\n" if $debug;
